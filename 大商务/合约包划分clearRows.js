$NG.AllReady(function (
		...
		useValuesChange(({ args }) => {
			console.log("contractMode:", args);
			// 这里可以添加其他需要在phidPc变化时执行的逻辑
		}, "contractMode");
		setTimeout(() => {
			const Oldrows = dgrid.getRows();
			console.log("初始行数据:", Oldrows);
		}, 2000);
		// 监听按钮点击事件
					...
					if (!rootNode.s_tree_pid || rootNode.s_tree_pid === '0' || rootNode.s_tree_pid === 0) {
						...
						data.forEach((dataItem) => {
							...
							if (rootNode.contPackCode === fbms) {
								...
								console.log("匹配的rootNode:",rootNode);
								dataAdded = true;
								...
								const childNode = {
									...
									contPackName: dataItem.extendObjects.cont_pack_name,
								...
								rootNode.children.push(childNode);
								// 更新根节点
								//dgrid.addChildrenRow(childNode);
								const newRows = dgrid.getRows();
								console.log("添加子节点后:",dgrid.getRows());
								dgrid.clearRows();
								console.log("清空行数据后:",dgrid.getRows());
								dgrid.addRows(newRows);
								console.log("劳务分包children",dgrid.getRows()[0].children);
								console.log("专业分包children",dgrid.getRows()[1].children);
								dgrid.refreshView();
							}
						});
					}
