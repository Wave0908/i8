    ...
    */
    ...
    if (otype == $Otype.EDIT) {
        mstform.on('dataready', function () {
            xmmc_first = "'" + mstform.getItem("xmmc").getValue() + "'";
            console.log("xmmc_first edit:", xmmc_first);
        })
    }
    if (otype == $Otype.ADD || otype == $Otype.EDIT) {
        ...
        mstform.getItem('u_xmqdxm').addListener('helpselected', async function () {
            console.log("xmmc_first add edit:", xmmc_first);
            var phid = mstform.getItem('u_xmqdxm').getValue();
            console.log(phid)
            await execServer('p_form0000000052_xmqd', {
                'phid': phid
            }, function (res) {
                console.log(res);
                if (res.count > 0) {
                    const data = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
                    console.log("p_form0000000052_xmqd data:", data);
            ...
            //sfbgz 2026.5.8 经查发现扩展名称有误 未生效，而扩展中存在着一个名为yfbgz的扩展未使用
            await execServer('sfbgz', {
                'phid': phid
            }, function (res) {
                console.log("sfbgz:", res);
                if (res.count > 0) {
            ...
            if (xmmc_val != xmmc_first) {
                await execServer('p_form0000000052_select_XMBM_list', {
                    'xmmc': xmmc_val
                }, function (res) {
                    if (res.count > 0) {
                        const data = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
