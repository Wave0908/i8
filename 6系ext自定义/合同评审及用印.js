...
function beforeUnVerifyList() { //单据列表去审核时检测
    var mgrid = Ext.getCmp('p_form0000000042_m');
    ...
    var ybyy;
    var xm;
    execServer("p_form0000000042_scjc", {
        'phid': ddbh
    }, (res) => { //调用注册SQL函数 HZJC 变量 htbh 赋值给函数变量ddbh,结果返回到res
        const data = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
        console.log("ybyy:", ybyy);
        ybyy = data[0].extendObjects.sl; //res返回值第一行 sl 字段值赋值给 ybyy
    });

    execServer("p_form0000000042_xmjc", {
        'pc': ddbh
    }, (res) => { //调用注册SQL函数 HZJC 变量 htbh 赋值给函数变量ddbh,结果返回到res
        const data = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
        xm = data[0].extendObjects.sl; //res返回值第一行 sl 字段值赋值给 ybyy
    });
...
function beforeSaveEdit() {
    ...
    if (mstform.getItem('sfbcxy').getValue() == 1) {//是补充协议
        if (mstform.getItem('bg_cnt_no').getValue() == '') {//补充协议编号不为空
            ...
            //生成变更评审编号
            execServer("p_form0000000042_bg_no", {
                'bill_no': no
            }, (res) => {
                const data = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
                console.log("data[0].extendObjects.流水号:", data[0].extendObjects.流水号);
                console.log("mstform.getItem('cnt_no').getValue() + 'B' + String(101 + data[0].extendObjects.流水号:", mstform.getItem('cnt_no').getValue() + 'B' + String(101 + data[0].extendObjects.流水号)
                    .substr(1, 2));
                mstform.getItem('bg_cnt_no').setValue(mstform.getItem('cnt_no').getValue() + 'B' + String(101 + data[0].extendObjects.流水号)
                    .substr(1, 2));
            });
            //return true;
        } else {
    ...
    //获取工具栏容器
    //获取表体容器
    var dgrid = Ext.getCmp('p_form0000000042_d');
    var dstore = dgrid.store;
    //获取表体1容器
    var dgrid1 = Ext.getCmp('p_form0000000042_d1');
    var dstore1 = dgrid1.store;
    //获取表体2容器
    var dgrid2 = Ext.getCmp('p_form0000000042_d2');
    var dstore2 = dgrid2.store;
    //获取表体3容器
    var dgrid3 = Ext.getCmp('p_form0000000042_d3');
    var dstore3 = dgrid3.store;
    //获取表体4容器
    var dgrid4 = Ext.getCmp('p_form0000000042_d4');
    var dstore4 = dgrid4.store;
    //获取表体5容器
    var dgrid5 = Ext.getCmp('p_form0000000042_d5');
    var dstore5 = dgrid5.store;
    //获取表体6容器
    var dgrid6 = Ext.getCmp('p_form0000000042_d6');
    var dstore6 = dgrid6.store;
    //获取表体7容器
    var dgrid7 = Ext.getCmp('p_form0000000042_d7');
    var dstore7 = dgrid7.store;
    //获取表体8容器
    var dgrid8 = Ext.getCmp('p_form0000000042_d8');
...
function getSaveDataEdit(type) {
    ...
    // 返回空对象而不是undefined，避免在edit.js中调用indexOf时出现TypeError
    return {};
    //return;
}
...
function allReadyEdit() { //页面编辑方法，固定写法
    ...
    console.log("mstform.getItem('htbm'):", mstform.getItem('htbm'));
    ...
    if (otype == $Otype.ADD || otype == $Otype.EDIT) {
        ...
        mstform.getItem('sfbcxy').addListener('change', function () { //监听表头sfbcxy字段值发生变化后立即触发
            ...
            if (mstform.getItem('sfbcxy').getValue() == 2) {
                ...
                mstform.getItem('cnt_no').userSetMustInput(false);
                ...
                mstform.getItem('htbm').addListener('helpselected', function () {
                    ...
                    execServer("p_form0000000042_bg_ht", {
                        'bill_no': htmc
                    }, (res) => {
                        console.log("res:", res);
                        //需要转json
                        ...
                        console.log("data.extendObjects:", data[0].extendObjects);
                        mstform.getItem('htzt').setValue(data[0].extendObjects.合同主体);
                        mstform.getItem('pc_name').setValue(data[0].extendObjects.工程项目);
                        mstform.getItem('xmgn').setValue(data[0].extendObjects.项目功能);
                        mstform.getItem('xmlx').setValue(data[0].extendObjects.工程类型);
                        mstform.getItem('bill_name').setValue(data[0].extendObjects.合同名称);
                        mstform.getItem('cnt_no').setValue(data[0].extendObjects.合同编码);
                        console.log("data[0].extendObjects.合同编码:", data[0].extendObjects.合同编码);
                        mstform.getItem('jfdw').setValue(data[0].extendObjects.甲方单位信息);
                        mstform.getItem('numericcol_1').setValue(data[0].extendObjects.签约合同金额);
                        mstform.getItem('userhelp_1').setValue(data[0].extendObjects.承揽模式);
                        mstform.getItem('htlx').setValue(data[0].extendObjects.合同类型);
                        mstform.getItem('htlrl').setValue(data[0].extendObjects.投标收益率);
        ...
        mstform.getItem('pc_name').addListener('helpselected', function () {
            ...
            execServer("p_form0000000042_pc_name", {
                'phid': pc_no
            }, (res) => {
                //给字段赋值
                const data = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
                var b = data[0].extendObjects.甲方单位信息
                mstform.getItem('gcdd').setValue(data[0].extendObjects.工程地点);
                mstform.getItem('country').setValue(data[0].extendObjects.国家);
                mstform.getItem('province').setValue(data[0].extendObjects.省);
                mstform.getItem('city').setValue(data[0].extendObjects.市);
                mstform.getItem('region').setValue(data[0].extendObjects.区县);
                //mstform.getItem('cnt_no').setValue(data[0].extendObjects.编码);
                mstform.getItem('userhelp_1').setValue(data[0].extendObjects.承揽模式);
                mstform.getItem('htlx').setValue(data[0].extendObjects.合同类型);
                mstform.getItem('xmlx').setValue(data[0].extendObjects.项目类型);
                mstform.getItem('htzt').setValue(data[0].extendObjects.承揽单位编码);
                mstform.getItem('xmgn').setValue(data[0].extendObjects.项目功能);
                mstform.getItem('jfdw').setValue(data[0].extendObjects.甲方单位信息);
                //编码转换为名称
                BatchBindCombox([mstform.getItem('country'), mstform.getItem('province'), mstform.getItem('city'), mstform.getItem('region')]);
        ...
        mstform.getItem('htzt').addListener('helpselected', function () {
            ...
            if (Ext.isEmpty(dwdm)) {
                ...
                //通过SQL赋值给承揽单位编码
                execServer("p_form0000000042_fbf", {
                    'phid': dwdm
                }, (res) => {
                    const data = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
                    var b = data[0].extendObjects.user_cldwbm;
