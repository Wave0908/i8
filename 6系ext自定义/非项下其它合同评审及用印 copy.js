    ...
    if (mstform.getItem('sfbcxy').getValue() == '2') {
        if (mstform.getItem('cnt_no').getValue() == '') {
            ...
            callServer('cnt_no', [{
                ...
                mstform.getItem('cnt_no').setValue(ssdwbm + cnt_typeno + year+String(1001 + res.record[0].流水号).substr(1, 3));
            });
            return true;
        } else {
            return true;
        ...
        callServer('bg_cntno', [{
            ...
            mstform.getItem('bg_cntno').setValue(mstform.getItem('cnt_no').getValue()+'B'+String(101 + res.record[0].流水号).substr(1, 2));
        })
        return true;
        } else {
            return true;
...
function allReadyEdit() { //页面编辑方法，固定写法
    ...
    //获取表体1容器
    var dgrid1 = Ext.getCmp('p_form0000000156_d1grid');
    var dstore1 = dgrid1.store;
    //获取表体2容器
    var dgrid2 = Ext.getCmp('p_form0000000156_d2grid');
    var dstore2 = dgrid2.store;
    //获取表体3容器
    var dgrid3 = Ext.getCmp('p_form0000000156_d3grid');
    var dstore3 = dgrid3.store;
    //获取表体4容器
    var dgrid4 = Ext.getCmp('p_form0000000156_d4grid');
    var dstore4 = dgrid4.store;
    //获取表体5容器
    var dgrid5 = Ext.getCmp('p_form0000000156_d5grid');
    var dstore5 = dgrid5.store;
    //获取表体6容器
    var dgrid6 = Ext.getCmp('p_form0000000156_d6grid');
    var dstore6 = dgrid6.store;
    //获取表体7容器
    var dgrid7 = Ext.getCmp('p_form0000000156_d7grid');
    var dstore7 = dgrid7.store;
    //获取表体容器
    var dgrid = Ext.getCmp('p_form0000000156_d8grid');
  ...
  mstform.getItem('amt').setValue(mstform.getItem('bg_amt').getValue()+mstform.getItem('numericcol_1').getValue());
    ...
    mstform.getItem('sfbcxy').addListener('change', function () { //监听表头是否补充协议字段值发生变化后立即触发 
        if (mstform.getItem('sfbcxy').getValue() == 2) {
            ...
            mstform.getItem('pc').userSetReadOnly(false); //项目名称不保护
            ...
            mstform.getItem('pc').userSetReadOnly(true); //项目名称保护
            ...
            mstform.getItem('yhtbh').addListener('helpselected', function () {
                ...
                callServer('bg', [{
                    ...
                    mstform.getItem('pc').setValue(res.record[0].项目);
                    mstform.getItem('sszz').setValue(res.record[0].实施组织);
                    mstform.getItem('cnt_no').setValue(res.record[0].合同编码);
                    mstform.getItem('title').setValue(res.record[0].合同名称);
                    mstform.getItem('cnt_type').setValue(res.record[0].合同类型);
                    mstform.getItem('numericcol_1').setValue(res.record[0].合同金额);
                    mstform.getItem('fbms').setValue(res.record[0].分包模式);
                    mstform.getItem('userhelp_1').setValue(res.record[0].结算方式);
                    mstform.getItem('fkfs').setValue(res.record[0].付款方式);
                    mstform.getItem('fkbl').setValue(res.record[0].付款比例);
                    mstform.getItem('sx').setValue(res.record[0].事项);
                    mstform.getItem('ssdwbm').setValue(res.record[0].实施单位编码);
                    mstform.getItem('org_id').setValue(res.record[0].供应商);
                })

                BatchBindCombox([mstform.getItem('pc'), mstform.getItem('sszz'), mstform.getItem('cnt_type'), mstform.getItem('userhelp_1'), mstform.getItem('org_id')])
            })
    ...
    mstform.getItem('sszz').addListener('helpselected', function () {
        ...
        callServer('fbf', [{
            ...
            mstform.getItem('ssdwbm').setValue(res.record[0].user_cldwbm);
        })
    })
    //选择合同类型更新合同类型编码
    mstform.getItem('cnt_type').addListener('helpselected', function () {
        var typeno = mstform.getItem('cnt_type').getValue();
        callServer('cnt_typeno', [{
            'phid': typeno
        }], function (res) {
            mstform.getItem('cnt_typeno').setValue(res.record[0].cnt_code);
        })
    })
    ...
    mstform.getItem('pc').addListener('helpselected', function () {
        ...
        var typeno = mstform.getItem('pc').getValue();
        callServer('pc_no', [{
            'phid': typeno
        }], function (res) {
            mstform.getItem('pc_no').setValue(res.record[0].pc_no);
        })
    })
