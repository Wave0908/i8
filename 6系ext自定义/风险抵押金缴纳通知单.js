function allReadyEdit() {
    ...
    var grid = Ext.getCmp('p_form0000700707_d');
    var dstore = grid.store;
    mstform.getItem('u_gcmc').addListener('change', function () {
        var u_gcmc = mstform.getItem('u_gcmc').getValue();
        console.log("u_gcmc:", u_gcmc);
        execServer('p_form0000700707_xmb', {
            phid: u_gcmc,
        },
            function (res) {
                if (res.count > 0) {
                    const data1 = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
                    var bm = data1[0].extendObjects.user_pc_dept;
                    mstform.getItem('u_ssxmb').setValue(bm);
                    BatchBindCombox([mstform.getItem('u_ssxmb')]);
                    var u_ssxmb = mstform.getItem('u_ssxmb').getValue();
                    console.log("u_ssxmb:",u_ssxmb);
                }
                // if (mstform.getItem('u_ssxmb').getValue()) {
                //     execServer('p_form0000700707_xmbry', {
                //         'phid': u_ssxmb,
                //     }, function (res1) {
                ...
                //             // 统一处理单个记录或数组
                //             var records = Array.isArray(data1) ? data1 : [data1];

                //             records.forEach(function (record) {
                //                 // 创建处理后的记录对象，只包含需要的字段
                //                 var processedRecord = {
                //                     u_gw: record.extendObjects.u_gw,
                //                     u_xzdbh: record.extendObjects.u_xzdbh,
                //                     u_ygxz: record.extendObjects.u_ygxz,
                //                     u_ygxz_name: record.extendObjects.u_ygxz_name
                //                 };
