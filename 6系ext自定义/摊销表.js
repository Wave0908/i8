function allReadyEdit() {
    ...
    dgrid.getColumn('u_bm_EXName').getEditor().on('helpselected', function (eOp, ignoreBeforeEvent) {
        ...
        console.log("u_bm:", u_bm);
        if (u_bm) {
            execServer('p_form0000600080_ngdept', {
                dept: u_bm
            }, function (res) {
                console.log("p_form0000600080_ngdept res:", res);
                if (res.count > 0) {
                    const data1 = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
                    console.log("user_lcywdy data1:", data1);
                    if (data1.length == 1) {
            ...
            execServer('p_form0000600080_ngdeptlchsbm', {
                ...
                console.log("p_form0000600080_ngdeptlc res:", res);
                if (res.count > 0) {
                    const data1 = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
                    console.log("u_mnemcodeinaccdepart data1:", data1);
                    if (data1.length == 1) {
