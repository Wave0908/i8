function beforeSaveEdit() {
    var mstform = Ext.getCmp('p_form0000000014_m');
    ...
    if (mstform.getItem('sfbcxy').getValue() == '2') {
        if (mstform.getItem('cnt_no').getValue() == '' || mstform.getItem('cnt_no').getValue() == null) {
            var no = mstform.getItem('bill_no').getValue();
            console.log("no:", no);
            console.log("ssdwbm:", ssdwbm);
            console.log("cnt_typeno:", cnt_typeno);
            //生成评审编号
            if (no && ssdwbm && cnt_typeno) {
                execServer('p_form0000000014_cnt_no', {
                    'pc_no': no,
                    'ssdwbm': ssdwbm,
                    ...
                    if (res.count > 0) {
                        ...
                        mstform.getItem('cnt_no').setValue(pc_no + ssdwbm + cnt_typeno + String(1001 + data[0].extendObjects.流水号).substr(1, 3));
                    }
                });
            }
            return true;
        ...
        if (mstform.getItem('bg_cntno').getValue() == '' || mstform.getItem('bg_cntno').getValue() == null) {
            var yhtbh = mstform.getItem('yhtbh').getValue();
            //生成变更评审编号
            if (yhtbh) {
                execServer('p_form0000000014_bg_cntno', {
                    'phid': yhtbh,
                }, function (res) {
                    if (res.count > 0) {
                        const data = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
                        mstform.getItem('bg_cntno').setValue(mstform.getItem('cnt_no').getValue() + 'B' + String(101 + data[0].extendObjects.流水号).substr(1, 2));
                    }
...
function allReadyEdit() { //页面编辑方法，固定写法
    ...
    //获取表体1容器
    var dgrid1 = Ext.getCmp('p_form0000000014_d');
    var dstore1 = dgrid1.store;
    //获取表体2容器
    // var dgrid2 = Ext.getCmp('p_form0000000014_d2');
    // var dstore2 = dgrid2.store;
    //获取表体3容器
    // var dgrid3 = Ext.getCmp('p_form0000000014_d3');
    // var dstore3 = dgrid3.store;
    //获取表体4容器
    // var dgrid4 = Ext.getCmp('p_form0000000014_d4');
    // var dstore4 = dgrid4.store;
    //获取表体5容器
    // var dgrid5 = Ext.getCmp('p_form0000000014_d5');
    // var dstore5 = dgrid5.store;
    //获取表体6容器
    // var dgrid6 = Ext.getCmp('p_form0000000014_d6');
    // var dstore6 = dgrid6.store;
    //获取表体7容器
    // var dgrid7 = Ext.getCmp('p_form0000000014_d7');
    // var dstore7 = dgrid7.store;
    //获取表体容器
    // var dgrid = Ext.getCmp('p_form0000000014_d8');
    // var dstore = dgrid.store;
    mstform.getItem('u_jymlja').setVisible(false);//甲目录
    mstform.getItem('u_jymlyi').setVisible(false);//乙目录


    mstform.getItem('numericcol_2').setVisible(false);
    mstform.getItem('numericcol_3').setVisible(false);
    mstform.getItem('cnt_type').addListener('change', function () {
        var cnt_type = mstform.getItem('cnt_type').getValue();
        if (cnt_type == '2') {
            mstform.getItem('numericcol_2').setVisible(true);
            //mstform.getItem('numericcol_2').userSetMustInput(true);
            mstform.getItem('numericcol_3').setVisible(true);
            //mstform.getItem('numericcol_3').userSetMustInput(true);
            //mstform.getItem('u_dyzbsr').userSetMustInput(true);

        } else if (cnt_type == '224200106000001') {
            mstform.getItem('numericcol_2').setVisible(true);
            //mstform.getItem('numericcol_2').userSetMustInput(true);
            mstform.getItem('numericcol_3').setVisible(true);
            //mstform.getItem('numericcol_3').userSetMustInput(true);
            ...
            mstform.getItem('numericcol_2').setVisible(false);
            mstform.getItem('numericcol_2').userSetMustInput(false);
            mstform.getItem('numericcol_3').setVisible(false);
            mstform.getItem('numericcol_3').userSetMustInput(false);
            mstform.getItem('u_dyzbsr').userSetMustInput(false);
        }
    ...
    if (otype == $Otype.ADD || otype == $Otype.EDIT || otype == $Otype.VIEW) {
        //线上线下显示甲乙目录
        mstform.getItem('u_sfcds').addListener('change', function () {
            var sfcds = mstform.getItem('u_sfcds').getValue()
            if (sfcds == 1) {
                mstform.getItem('u_jymlyi').setValue('');
                mstform.getItem('u_jymlyi').setVisible(false);
                mstform.getItem('u_jymlja').setValue('');
                mstform.getItem('u_jymlja').setVisible(false);
                mstform.getItem('u_jymlyi').userSetMustInput(false);
                mstform.getItem('u_jymlja').userSetMustInput(false);
            } else if (sfcds == 2) {
                mstform.getItem('u_jymlyi').setVisible(false);
                mstform.getItem('u_jymlyi').setValue('');
                mstform.getItem('u_jymlja').setVisible(true);
                mstform.getItem('u_jymlja').setValue('');
                mstform.getItem('u_jymlyi').userSetMustInput(false);
                mstform.getItem('u_jymlja').userSetMustInput(true);
            } else if (sfcds == 3) {
                mstform.getItem('u_jymlyi').setValue('');
                mstform.getItem('u_jymlyi').setVisible(true);
                mstform.getItem('u_jymlja').setValue('');
                mstform.getItem('u_jymlja').setVisible(false);
                mstform.getItem('u_jymlyi').userSetMustInput(true);
                mstform.getItem('u_jymlja').userSetMustInput(false);
            } else {
                mstform.getItem('u_jymlyi').setValue('');
                mstform.getItem('u_jymlyi').setVisible(false);
    ...
    mstform.getItem('bglx').setVisible(false);
    ...
    //合同签约日期为必输
    mstform.getItem('datetimecol_2').userSetMustInput(true);
    //盖章份数为必输
    mstform.getItem('gzfs').userSetMustInput(true);
    //合同章号为必输
    mstform.getItem('htzh').userSetMustInput(true);


    if (otype == $Otype.ADD || otype == $Otype.EDIT || otype == $Otype.COPY) {
        mstform.getItem('bg_amt').addListener('change', function () {
            mstform.getItem('amt').setValue(parseFloat(mstform.getItem('bg_amt').getValue()) + parseFloat(mstform.getItem('numericcol_1').getValue()));
        });
        mstform.getItem('sfbcxy').addListener('change', function () { //监听表头是否补充协议字段值发生变化后立即触发 
            if (mstform.getItem('sfbcxy').getValue() == 2) {
                ...
                mstform.getItem('bg_amt').setVisible(false);
                mstform.getItem('bglx').setVisible(false);
                mstform.getItem('cnt_no').userSetMustInput(true); //合同编码可见  cnt_no
                mstform.getItem('cnt_no').setVisible(true); //合同编码可见  cnt_no
                ...
                mstform.getItem('vendor_id').userSetReadOnly(false);
                ...
                mstform.getItem('bglx').setVisible(true);
                ...
                mstform.getItem('vendor_id').userSetReadOnly(true);
                ...
                mstform.getItem('yhtbh').addListener('helpselected', function () {
                    ...
                    execServer('p_form0000000014_bg', {
                        'phid': phid
                    }, function (res) {
                        console.log(res);
                        const data = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
                        ...
                        mstform.getItem('numericcol_1').setValue(data[0].extendObjects.结算式);
                        mstform.getItem('fbms').setValue(data[0].extendObjects.分包模式);

                        //mstform.getItem('numericcol_1').setValue(res.record[0].sqbg);
                        ...
                        mstform.getItem('vendor_id').setValue(data[0].extendObjects.供应商);
                    })

                    BatchBindCombox([mstform.getItem('phid_pc'), mstform.getItem('sszz'), mstform.getItem('cnt_type'), mstform.getItem('numericcol_1'), mstform.getItem('vendor_id')])
                })
        ...
        mstform.getItem('sszz').addListener('helpselected', function () {
            ...
            mstform.getItem('phid_org').setValue(dwdm)
            BatchBindCombox([mstform.getItem('phid_org').setValue(dwdm)])
            execServer('p_form0000000014_fbf', {
                'phid': dwdm
            }, function (res) {
                if (res.count > 0) {
                    const data = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
        ...
        mstform.getItem('cnt_type').addListener('helpselected', function () {
            ...
            execServer('p_form0000000014_cnt_typeno', {
                'phid': typeno
            }, function (res) {
                if (res.count > 0) {
                    const data = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
        ...
        mstform.getItem('phid_pc').addListener('helpselected', function () {
            ...
            execServer('p_form0000000014_pc_no', {
                'phid': typeno
            }, function (res) {
                if (res.count > 0) {
                    const data = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
