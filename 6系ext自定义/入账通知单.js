function allReadyEdit() {
    ...
    var dgrid = Ext.getCmp('p_form0000000019_d');
    var dgrid1 = Ext.getCmp('p_form0000000019_d1');
    var Toolbar = Ext.getCmp('toolbar');
    var dstore = dgrid.store;
    dgrid.hideColumn('htbh', true);
    dgrid.hideColumn('zt', true);
    dgrid.hideColumn('gys', true);
    console.log('bill_name=================>', mstform.getItem('bill_name'))
    ...
    Toolbar.items.get('push').on('click', function () {
        Ext.Ajax.request({
            ...
            url: "http://172.20.65.5:30599/new_esey/lcApi/pushRztzToLc?phid=" + busid,
            async: false, //同步请求
    ...
    Toolbar.items.get('check').on('click', function () {
        var currRows = dgrid.getStore().getRange(0, dstore.getCount() - 1);
        console.log("currRows.length=================>", currRows.length)
        console.log("currRows=================>", currRows)
        var zclx = mstform.getItem('zclx').getValue();
        if (zclx == '4') {
            for (var j = 0; j < currRows.length; j++) {
                console.log("zcbm=================>", currRows[j].get('zcbm'))
                if (!currRows[j].get('zcbm')) {
                    Ext.MessageBox.confirm('提示', '请先点击保存，生成资产编码', function (e) {
    ...
    dgrid1.getColumn('kxxz_EXName').getEditor(obj).addListener('helpselected', function () {
    ...
    if (otype == $Otype.ADD || otype == $Otype.EDIT || otype === 'copy') {
        ...
        Toolbar.get('myadd').on('click', function () {
            //判断项目信息是否选择 
            if (Ext.isEmpty(mstform.getItem('userhelp_1').getValue())) {
                //未选择提示：请先选择项目！
            ...
            if (Ext.isEmpty(a.length >= 1)) {
                var help = Ext.create('Ext.ng.MultiRichHelp', {
                    ...
                    outFilter: {
                        'pphid': mstform.getItem('userhelp_1').getValue()
                    },
        ...
        mstform.getItem('userhelp_1').on('beforetriggerclick', function (eOp, ignoreBeforeEvent) {
            ...
            var zclx = mstform.getItem('zclx').getValue();
            var glbm = mstform.getItem('glbm').getValue();
            if (Ext.isEmpty(glbm) || Ext.isEmpty(phid_org) || Ext.isEmpty(zclx)) {
        ...
        mstform.getItem('glbm').addListener('helpselected', function () {
            var glbm = mstform.getItem('glbm').getValue();
            if (glbm) {
        ...
        //根据设备类别过滤主类别
        dgrid.getColumn('zclb_EXName').getEditor().on('beforetriggerclick', function (eOp, ignoreBeforeEvent) {
            var cc = mstform.getItem('zclx').getValue();
            //console.log(cc);
            dgrid.getColumn('zclb_EXName').getEditor().setOutFilter({
                'sblx': cc
            });
        })
        //根据主类别过滤辅类别
        dgrid.getColumn('zcfl_EXName').getEditor().on('beforetriggerclick', function (eOp, ignoreBeforeEvent) {

            var data = dgrid.getSelectionModel().getSelection();
            var dd = data[0].get('zclb');
            var fl;
            execServer('p_form0000000019_zcfl', {
                'A': dd
            }, function (res) {
                const data = JSON.parse(res.data);
                fl = data[0].c_no
            });
            dgrid.getColumn('zcfl_EXName').getEditor().setOutFilter({
                'c_descript': fl
            });
        })
        //一建复制功能
        ...
        Toolbar.items.get('copy').on('click', function () {
            ...
            var zl = a[0].get('zclb');
            var zl_name = a[0].get('zclb_EXName');
            var fl = a[0].get('zcfl');
            var fl_name = a[0].get('zcfl_EXName');
            var lyfs = a[0].get('zclyfs');
            var lyfs_name = a[0].get('zclyfs_EXName')
            var dwmc = a[0].get('org_id1');
            var dwmc_name = a[0].get('org_id1_EXName');
            var dw = a[0].get('msunit');
            var dw_name = a[0].get('msunit_EXName');
            ...
            var org_id_name = a[0].get('org_id_EXName');
            for (j = 1; j < a.length; j++) {
                a[j].set('pphid', pphid);
                a[j].set('zcmc', zcmc);
                a[j].set('ggxh', ggxh);
                a[j].set('zclb', zl);
                a[j].set('zclb_EXName', zl_name);
                a[j].set('zcfl', fl);
                a[j].set('zcfl_EXName', fl_name);
                a[j].set('zclyfs', lyfs);
                a[j].set('zclyfs_EXName', lyfs_name);
                a[j].set('org_id1', dwmc);
                a[j].set('org_id1_EXName', dwmc_name);
                a[j].set('msunit', dw);
                a[j].set('msunit_EXName', dw_name);
                a[j].set('qty', qty);
                a[j].set('qty_1', sl);
                a[j].set('prc', prc);
                a[j].set('amt', amt);
                ...
                a[j].set('org_id_EXName', org_id_name);
            }
            //临设类
            if (lb == '1') {
                var zclb = a[0].get('zclb')
                var zclb_name = a[0].get('zclb_EXName')
                var zclyfs = a[0].get('zclyfs')
                var zclyfs_name = a[0].get('zclyfs_EXName')
                var szrq = a[0].get('szrq')
                var qyrq = a[0].get('qyrq')
                var syzt = a[0].get('syzt')
                var org_id = a[0].get('org_id')
                ...
                for (i = 1; i < a.length; i++) {
                    a[i].set('zclb', zclb);
                    a[i].set('zclb_EXName', zclb_name);
                    a[i].set('zclyfs', zclyfs);
                    a[i].set('zclyfs_EXName', zclyfs_name);
                    a[i].set('szrq', szrq);
                    a[i].set('qyrq', qyrq);
                    a[i].set('syzt', syzt);
                    a[i].set('org_id', org_id);
                    a[i].set('org_id_EXName', org_id_name);
                }
            }
            //房产类
            if (lb == '2') {
                ...
                var gys_name = a[0].get('vendor_id_EXName');
                ...
                for (i = 1; i < a.length; i++) {
                    ...
                    a[i].set('vendor_id_EXName', gys_name);
                    a[i].set('sjzyr', sjzyr);
    ...
    if (otype == $Otype.ADD || otype == $Otype.EDIT || otype === 'copy') {
        ...
        mstform.on('dataready', function () {
            ...
            execServer('p_form0000000019_zcbm', {
                'bill_no': "'" + bill_no + "'"
            }, function (res) {
                const data = JSON.parse(res.data);
                for (var i = 0; i < res.count; i++) {
                    arr.push({
                        zcbm: data[i].extendObjects.zcbm
                    });
                }
            });
            ...
            dgrid.addListener('edit', function (editor, e) {
                ...
                if (e.field == 'zcbm') { //监听qty、prc字段变化
                    ...
                    // 新增：校验当前单据内各行的 zcbm 不允许重复
                    var currRows = dgrid.getStore().getRange(0, dstore.getCount() - 1);
                    var currIndex = dstore.indexOf(record);
                    for (var j = 0; j < currRows.length; j++) {
                        if (j === currIndex) continue; // 跳过当前行
                        if (currRows[j].get('zcbm') == zcbm && !Ext.isEmpty(zcbm)) {
                            Ext.Msg.alert('提示', '该资产编码在当前单据中重复');
                            record.set('zcbm', null);
                            return false;
                        }
                    }
...
function attachReturnExt(key, value) {
    ...
    if (key == 'closeNG3Container') {
        if (value == '') {
            ...
            mstform.getItem('fj_num').setValue(res.count);
        }
    } else {
        return;
    }
...
function beforeSaveEdit() {
    ...
    if (otype == $Otype.ADD) {
        if (zclx == '4') {
            ...
            execServer('p_form0000000019_mywxzc', {
                'year': year1,
                'zclx': zclx1
            }, function (res) {
                if (!Ext.isEmpty(res.data)) {
                    execServer('p_form0000000019_pt_ne_ass', {
                        'year': year1,
                        'zclx': zclx1
                    }, function (res) {
                        const data = JSON.parse(res.data);
                        if (res.count == 1) {
                            var temp;
                            console.log(data[0].extendObjects);
                            var ser_number = data[0].extendObjects.ser_number
    ...
    if (otype == $Otype.ADD || otype == $Otype.EDIT) {
        var rows = dstore.getCount();
        if (rows < 1) {
            Ext.Msg.alert('提示', '请在往来明细表添加数据');
            flag = 0
            return false;
        } else {
            flag = 1
