function allReadyEdit() {
    ...
    if (!Ext.isEmpty(Toolbar) && otype == $Otype.VIEW) {
        ...
        Toolbar.items.get('push1').on('click', function () {
            ...
            Ext.Ajax.request({
                ...
                url: "http://192.168.3.241:30599/new_esey/lcApi/pushTXB?phid=" + busid,
                async: false, //同步请求
        ...
        Toolbar.items.get('push').on('click', function () {
            ...
            Ext.Ajax.request({
                ...
                dataType: 'json',
                contentType: 'application/json;charset=UTF-8',
                url: "http://192.168.3.241:30599/new_esey/lcApi/pushFLD?phid=" + busid,
                async: false, //同步请求
                disableCaching: false, // 添加这行来阻止_dc参数
                success: function (response) {
                    window.wait = false;
    ...
    mstform.on('dataready', function () {
        ...
        if (mstform.getItem('app_status').getValue() == '0') {
            ...
            if (mstform.getItem('u_istbinspur_fld').getValue() == '0' || mstform.getItem('u_istbinspur_fld').getValue() == '4' || mstform.getItem('u_istbinspur_fld').getValue() == null) {
                Toolbar.items.get('push').enable();
            } else {
                console.log("该单据同步浪潮物资发料单标志不为0、4或空,不能推送");
                Toolbar.items.get('push').disable();
            }
        }
    });
    ...
    mstform.getItem('u_txdlcglbm').addListener('helpselected', function () {
        ...
        execServer('ssxmb_zjm', {
            ...
            if (res.count > 0) {
                var data = JSON.parse(res.data);
                mstform.getItem('u_txdlchsbmzjm').setValue(data[0].extendObjects
                    .user_mnemcodeinaccdepart);
            } else {
                Ext.Msg.alert('提示', '浪潮核算部门的助记码没有和新中大部门编码保持一致');
                return false;
