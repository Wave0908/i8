...
function allReadyEdit() {
    ...
    //明细行采购合同物料名称为只读
    dgrid.setReadOnlyCol('u_cghtwlmc_EXName', true)
    //明细行采购合同剩余量为只读
    ...
    //明细项目需用计划名称为只读
    dgrid.setReadOnlyCol('u_xmxyjhmc_EXName', true)
    //明细采购计划数量为只读
    dgrid.setReadOnlyCol('u_cgjhsl', true)
    //明细采购计划名称为必填
    dgrid.setMustInputCol('u_cgjh_EXName', true)
    //明细采购计划物料名称为必填
    dgrid.setMustInputCol('u_cgjhwl_EXName', true)
    ...
    mstform.getItem('u_zzhtxx').addListener('beforetriggerclick', function (obj) {
        ...
        mstform.getItem('u_zzhtxx').setClientSqlFilter("pcm3_cnt_m.phid_org = " + ocode + " and pcm3_cnt_m.phid not in (select u_zzhtxx from p_form0000700172_m where app_status <> '1')");
    ...
    mstform.getItem('u_zzhtxx').addListener('helpselected', function (obj) {
        ...
        mstform.getItem('phid_pc').setValue(obj.data.phid_pc);
        BatchBindCombox([mstform.getItem('phid_pc')]);

        var u_zzhtxx = mstform.getItem('u_zzhtxx').getValue();
        if (u_zzhtxx) {
            execServer('p_form0000700172_cghtzz', {
                'phid': u_zzhtxx
            }, function (res) {
                if (res.count > 0) {
                    ...
                    for (let index = 0; index < res.count; index++) {
                        arr.push({
                            u_cghtwlbm: data1[index].extendObjects.u_cghtwlbm,
                            u_cghtwlmc_EXName: data1[index].extendObjects.u_cghtwlmc_name,
                            u_cghtwlmc: data1[index].extendObjects.u_cghtwlmc,
                            u_qty: data1[index].extendObjects.u_qty
                        });
                    }
                    dstore.removeAll();
    ...
    //采购计划选择前触发
    dgrid.getColumn('u_cgjh_EXName').getEditor().addListener('beforetriggerclick', function () {
        var pc = mstform.getItem('phid_pc').getValue();
        var data = dgrid.getSelectionModel().getSelection();
        dgrid.getColumn('u_cgjh_EXName').getEditor().setClientSqlFilter(" mpm.phid_pc= " + pc + " and mpd.phid_itemdata = " + data[0].get('u_cghtwlmc'));
    });
    ...
    //采购计划选择后触发
    dgrid.getColumn('u_cgjh_EXName').getEditor().addListener('helpselected', function (obj) {
        var data = dgrid.getSelectionModel().getSelection();
        data[0].set('u_cgjhbm', obj.data.bill_no);
    });
    ...
    //采购计划物料名称选择前触发
    dgrid.getColumn('u_cgjhwl_EXName').getEditor().addListener('beforetriggerclick', function () {
        ...
        dgrid.getColumn('u_cgjhwl_EXName').getEditor().setClientSqlFilter(" mpm.phid= " + data[0].get('u_cgjh') + " and mpd.phid_itemdata = " + data[0].get('u_cghtwlmc'));
    });
    ...
    //采购计划物料名称选择后触发
    dgrid.getColumn('u_cgjhwl_EXName').getEditor().addListener('helpselected', function (obj) {
        console.log(obj);
        var data = dgrid.getSelectionModel().getSelection();
        data[0].set('u_xmxyjhbh', obj.data.bill_no);
        data[0].set('u_xmxyjhmc', obj.data.pphid);
        data[0].set('u_xmxyjhmc_EXName', obj.data.bill_name);
