function allReadyEdit() {
    ...
    var dgrid = Ext.getCmp('p_form0000600080_dgrid');
    var dstore = dgrid.store;
    var dgrid2 = Ext.getCmp('p_form0000600080_d1grid');
    ...
    mstform.on('dataready', function () {
        if (mstform.getItem('u_istbinspur_fld').getValue() == '1' || mstform.getItem('u_istbinspur')
            .getValue() == '1') {
            Ext.getCmp('toolbar').get('applycheck').disable()
        }
    });
    Toolbar.insert(1, {
        itemId: "copy",
        ...
        if (a.length == '0') {
            ...
            for (var i = 0; i < dstore.getCount(); i++) {
                ...
                callServer(
                    ...
                    function (res) {
                        a[i].set('ytxbl', res.record[0].ytxbl);
                        a[i].set('ytxje', res.record[0].ytxje);
                        a[i].set('u_ytxhsje', res.record[0].u_ytxhsje);
                    }
                );
            }
        }
    ...
    mstform.getItem('pc').addListener('beforetriggerclick', function () {
        ...
        mstform.getItem('pc').setOutFilter({
            cat_phid: pocode,
        });
    });
    /*工程项目字段通用帮助选择前触发 end*/
    ...
    mstform.getItem('htmc').addListener('helpselected', function () {
        ...
        callServer(
            ...
            function (res) {
                if (res.record[0]) {
                    mstform.getItem('htbm').setValue(res.record[0].htbm);
                    mstform
                        .getItem('vendor_id')
                        .setValue(res.record[0].sencomp);
                    BatchBindCombox([
                        mstform
                            .getItem('vendor_id')
                            .setValue(res.record[0].sencomp),
    ...
    if (imp_schemeid == '224210827000001' || imp_schemeid == '224220915002001' || imp_schemeid == '221019004002' ||
        ...
        //材料类别必输
        dgrid.setMustInputCol('u_cllb_name', true);
        //核算项目必输
        dgrid.setMustInputCol('u_hsxm_name', true);
        //专业工程必输
        dgrid.setMustInputCol('u_zygc_name', true);
        //板块必输
        dgrid.setMustInputCol('u_bk_name', true);
        //部门必输
        dgrid.setMustInputCol('u_bm_name', true);
        //方案  采购入库自有周转发料摊销单
    ...
    dgrid.getColumn('u_bm_name').getEditor().on('helpselected', function (eOp, ignoreBeforeEvent) {
        ...
        callServer('ngdeptlchsbm', [{
            ...
            if (res.record[0]) {
                data[0].set('u_mnemcodeinaccdepart', res.record[0].user_mnemcodeinaccdepart)
            }
        });
    });
    /*部门选择后根据组织过滤部门触发end*/
    ...
    document.getElementById('p_form0000600080_dgrid').onclick = function () {
        Toolbar.get('addrow').hide(); //隐藏增行 
    }
    document.getElementById('p_form0000600080_d1grid').onclick = function () {
        if (mstform.getItem('u_lcts').getValue() == '2') {
    ...
    Toolbar.items.get('copy').on('click', function () {
        ...
        var u_fyxmname = a[0].get('u_fyxm_name');
        //gs物资发料费用项目
        var u_wzfldfyxm = a[0].get('u_wzfldfyxm');
        var u_wzfldfyxmname = a[0].get('u_wzfldfyxm_name');
        //核算项目字段
        var u_hsxm = a[0].get('u_hsxm');
        var u_hsxmname = a[0].get('u_hsxm_name');
        //专业工程字段
        var u_zygc = a[0].get('u_zygc');
        var u_zygcname = a[0].get('u_zygc_name');
        //板块字段
        var u_bk = a[0].get('u_bk');
        var u_bkname = a[0].get('u_bk_name');
        //往来单位字段
        var u_wldw = a[0].get('u_wldw');
        var u_wldwname = a[0].get('u_wldw_name');
        //部门字段
        var u_bm = a[0].get('u_bm');
        var u_bmname = a[0].get('u_bm_name');
        //本次摊销比例
        var bctxbl = a[0].get('bctxbl');

        for (var i = 1; i < a.length; i++) {
            a[i].set('u_wzfldfyxm', u_wzfldfyxm);
            a[i].set('u_wzfldfyxm_name', u_wzfldfyxmname);

            a[i].set('u_fyxm', u_fyxm);
            a[i].set('u_fyxm_name', u_fyxmname);
            a[i].set('u_hsxm', u_hsxm);
            a[i].set('u_hsxm_name', u_hsxmname);
            a[i].set('u_zygc', u_zygc);
            a[i].set('u_zygc_name', u_zygcname);
            a[i].set('u_bk', u_bk);
            a[i].set('u_bk_name', u_bkname);
            a[i].set('u_wldw', u_wldw);
            a[i].set('u_wldw_name', u_wldwname);
            a[i].set('u_bm', u_bm);
            a[i].set('u_bm_name', u_bmname);
            if (!Ext.isEmpty(u_bm)) {
                callServer('ngdept', [{
                    dept: a[i].get('u_bm')
                }], function (res) {
                    if (res.record[0]) {
                        a[i].set('u_lcywdy', res.record[0].user_lcywdy)
                    }
                });
                callServer('ngdeptlchsbm', [{
                    dept: a[i].get('u_bm')
                    ...
                    if (res.record[0]) {
                        a[i].set('u_mnemcodeinaccdepart', res.record[0].user_mnemcodeinaccdepart)
                    }
                })
            }
    ...
    /*点验单浪潮管理组织部门自动带出业务单元，核算部门助记码start*/
    ...
    mstform.getItem('u_dydlcglbm').addListener('helpselected', function () {
        ...
        execServer('ssxmb_bmywdy', {
            ...
            if (res.data[0]) {
                mstform.getItem('u_dydlcywdy').setValue(res.data[0].user_lcywdy);
            } else {
        ...
        execServer('ssxmb_zjm', {
            ...
            if (res.data[0]) {
                mstform.getItem('u_dydlchsbmzjm').setValue(res.data[0]
    ...
    /*点验单浪潮管理组织部门自动带出业务单元，核算部门助记码end*/
    ...
    mstform.getItem('u_txdlcglbm').addListener('helpselected', function () {
        ...
        execServer('ssxmb_bmywdy', {
            ...
            if (res.data[0]) {
                mstform.getItem('u_txdlcywdy').setValue(res.data[0].user_lcywdy);
            } else {
                Ext.Msg.alert('提示', '请先维护部门业务单元对照表');
                return false;
            }
        ...
        execServer('ssxmb_zjm', {
            ...
            if (res.data[0]) {
                mstform.getItem('u_txdlchsbmzjm').setValue(res.data[0]
                    .user_mnemCodeInAccDepart);
            } else {
                Ext.Msg.alert('提示', '浪潮核算部门的助记码没有和新中大部门编码保持一致');
                return false;
...
function allReadyList() {
    Ext.getCmp('toolbar').get('applycheck').hide()
