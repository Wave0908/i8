function allReadyEdit() {
    ...
    var dgrid = Ext.getCmp('p_form0000000088_d1grid');
    var dgrid3 = Ext.getCmp('p_form0000000088_d3grid');
    var dgrid4 = Ext.getCmp('p_form0000000088_d4grid');
    //其他人数标签
    ...
    mstform.getItem('pxxmmc').on('beforetriggerclick', function(eOp, ignoreBeforeEvent) { //帮助窗口打开前事件
        ...
        var ocode_val = mstform.getItem('ocode').getValue();
    ...
    dgrid.addListener('edit', function(editor, e) { //监听单据体编辑状态，edit-编辑；add-增行；cellclick-点击
        ...
        if(e.field == 'empid') {
            ...
            callServer('epm', [{
                ...
                record.set('xb', res.record[0].sex);
                record.set('nl', res.record[0].age);
                record.set('lxdh', res.record[0].mobile1);
                record.set('zc', res.record[0].c_name);
            })
        };
    });
    ...
    dgrid4.addListener('edit', function(editor, e) { //监听单据体编辑状态，edit-编辑；add-增行；cellclick-点击
        ...
        if(e.field == 'u_ygbh') {
            ...
            callServer('epm', [{
                ...
                record.set('empid', res.record[0].phid);
            })
        };
    });
...
function beforeSaveEdit() {
    ...
    if(dstore4.getCount() < 1) {
        ...
        if(lx == '01') {
            var dgrid1 = Ext.getCmp('p_form0000000088_d1grid');
        ...
        if(lx == '02') {
            var dgrid2 = Ext.getCmp('p_form0000000088_d2grid');
