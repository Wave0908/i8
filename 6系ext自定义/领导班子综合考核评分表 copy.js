function allReadyEdit() {
    ...
    var d1grid = Ext.getCmp('p_form0000000263_d1grid');
    var d2grid = Ext.getCmp('p_form0000000263_d2grid');
    ...
    mstform.getItem('pfzq').addListener('helpselected', function () {
        if (!Ext.isEmpty(Ext.getCmp('pfzq').getValue())) {
            callServer('pdkhzqry', [{ ndfw: mstform.getItem('pfzq').getValue() }], function (res1) {
                //console.log(res1.record[0].sl);
                if (res1.record[0].sl == 0) {
    ...
    mstform.getItem('empid').addListener('helpselected', function (obj) {
        if (Ext.isEmpty(Ext.getCmp('pfzq').getValue())) {
            ...
            fillgrid(obj.code, obj.name, Ext.getCmp('pfzq').getValue());
            hidegridc();
        }
    ...
    function fillgrid(csygbm, csygxm, cspfzq) {
        callServer('sfyjl', [{ yhbm1: csygbm, kpnd1: cspfzq }], function (res) {
            var tmpcount = res.record[0].sfcz;
            if (tmpcount == 0) {
                callServer('GetKpdw', [{ kpnd: cspfzq }], function (res) {
                    mstform.getItem('title').setValue(cspfzq + '年度' + csygxm + '同志对相关单位领导班子综合考核评分表');
                    d1store.removeAll(); //清除单据体内所有数据
                    d1store.insert(d1store.getCount(), res.record); //将服务端获取的数组内容插入到单据体
                });
                callServer('GetKpzgxmb', [{ kpnd: cspfzq }], function (res) {
                    d2store.removeAll(); //清除单据体内所有数据
                    d2store.insert(d2store.getCount(), res.record); //将服务端获取的数组内容插入到单据体
                });
            }
            else {
                mstform.getItem('empid').setValue('');
    ...
    mstform.on('dataready', function () {
        ...
        //     hidegridc();                    
        // }  
        callServer('GetEpmId', [{ dlid: $appinfo.userID }], function (res) {

            dlid = res.record[0].hrid;
        });


        callServer('GetPfrDept', [{ epmids: dlid }], function (res) {

            pfrbm = res.record[0].dept;
        });
