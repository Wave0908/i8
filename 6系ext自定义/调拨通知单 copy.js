function allReadyEdit() {
    ...
    var dgrid = Ext.getCmp('p_form0000000236_d1grid');
    var dstore = dgrid.store;
    var dgrid1 = Ext.getCmp('p_form0000000236_d2grid');
    ...
    Toolbar.items.get('push').on('click', function() {
        Ext.Ajax.request({
            ...
            url: location.protocol + "//" + location.host + "/MCC22ToFSSC/api/Assets/Insert",
            async: false, //同步请求
    ...
    if(otype == $Otype.ADD || otype == $Otype.EDIT) { //var a = document.getElementById('zcxz-inputEl').set
        document.getElementById('zcxz-inputEl').style.backgroundColor = "darkorange";
        ...
        callServer('zz', [{
            ...
            mstform.getItem('title').setValue(res.record[0].deptname + '资产调拨单');
        });
        //浪潮核算部门过滤
        ...
        mstform.getItem('dbfs').addListener('change', function() {
            ...
            if(mstform.getItem('dbfs').getValue() == 1) {
                ...
                //字段资产增加方式为显示
                dgrid.hideColumn('zczjfs_name', false);
                //字段资产增加方式为必输项
                dgrid.setMustInputCol('zczjfs_name', true);
                //字段资产减少方式隐藏
                dgrid.hideColumn('zcjsfs_name', true);
                if(otype == $Otype.ADD) {

                    //单据编码规则
                    callServer('zcdr', [{}], function(res) {
                        var sum = res.record[0].sl;
                        if(0 <= sum && sum < 9) {
            ...
            //调出
                ...
                //字段资产减少方式为显示
                dgrid.hideColumn('zcjsfs_name', false);
                //字段资产减少方式为必输项
                dgrid.setMustInputCol('zcjsfs_name', true);
                //字段资产增加方式隐藏
                dgrid.hideColumn('zczjfs_name', true);
                if(otype == $Otype.ADD) {
                    //单据编码规则
                    callServer('zcdc', [{}], function(res) {
                        var sum = res.record[0].sl;
                        if(0 <= sum && sum < 9) {
            ...
            if(mstform.getItem('dbfs').getValue() == 1) {
                ...
                var help = Ext.create('Ext.ng.MultiRichHelp', { //创建通用帮助多选
                    ...
                    outFilter: {
                        ...
                        'dygocode': mstform.getItem('ocode').getValue()
            ...
            if(mstform.getItem('dbfs').getValue() == 2) {
                ...
                var help = Ext.create('Ext.ng.MultiRichHelp', { //创建通用帮助多选
                    ...
                    outFilter: {
                        ...
                        'ocode': mstform.getItem('ocode').getValue()
                    } //帮助对应过滤条件，帮助过滤字段等于表单字段  
                }); //创建通用帮助多选事件结束 ｝
                help.showHelp();
                help.on('helpselected', function(obj) { //通用帮助选择更新
                    ...
                    for(i = 0; i < obj.data.length; i++) { //for循环更新单据体数据 *obj.data.length表示选中帮助窗口的条数
                        arr.push({
                            ...
                            zczl_name: obj.data[i].data.zlb_name,
                            zcfl: obj.data[i].data.zcflb, //资产辅类别	
                            zcfl_name: obj.data[i].data.c_name,
                            //msunit_name: obj.data[i].data.msname, 									
                            gocode: obj.data[i].data.ocode, //管理组织		
                            gocode_name: obj.data[i].data.gname,
                            dcdept: obj.data[i].data.deptid, //管理部门
                            dcdept_name: obj.data[i].data.gdname,
                            dsocode: obj.data[i].data.user_ocode, //使用组织								
                            dsocode_name: obj.data[i].data.oname,
                            dsdept: obj.data[i].data.deptid2, //使用部门
                            dsdept_name: obj.data[i].data.deptname,
                            zclyfs: obj.data[i].data.zclyfs, //资产来源方式
                            zclyfs_name: obj.data[i].data.ly_name,
                            ccbh: obj.data[i].data.ccbh,
        ...
        //表体调入部门控制
        dgrid.getColumn('dygdept_name').getEditor().on('beforetriggerclick', function(eOp, ignoreBeforeEvent) {
            ...
            //console.log(cc);
            dgrid.getColumn('dygdept_name').getEditor().setOutFilter({
                'parent_orgid': cc
            });
        })

        //调入使用部门控制
        dgrid.getColumn('dysdept_name').getEditor().on('beforetriggerclick', function(eOp, ignoreBeforeEvent) {
            var data = dgrid.getSelectionModel().getSelection(); //获取当前选中行
            var cc = data[0].get('dysocode'); //获取当前选中行某个字段值
            //var cc = mstform.getItem('zclx').getValue();
            //console.log(cc);
            dgrid.getColumn('dysdept_name').getEditor().setOutFilter({
                'parent_orgid': cc
            });
        })
    }
