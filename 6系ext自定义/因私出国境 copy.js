function allReadyEdit() {
    ...
    mstform.getItem('userhelp_3').setVisible(false);
    mstform.getItem('textcol_1').setVisible(false);
    mstform.getItem('textcol_4').setVisible(false);
    mstform.getItem('userhelp_6').setVisible(false);
    mstform.getItem('checkboxcol_10').setVisible(false);
    mstform.getItem('checkboxcol_17').setVisible(false);
    mstform.getItem('userhelp_2').setVisible(false);
    mstform.getItem('u_jqzl').setVisible(false);
    ...
    mstform.getItem('rbcol_2').addListener('change', function (res) {
        var sfsmry = res.lastValue.rbcol_2;
    ...
    function loadEmployeeInfo() {
        ...
        callServer('ryxx', params, function (res) {
            // 检查数据是否丢失
            if (!res || !res.record || !res.record.length) {
                console.warn('员工信息查询返回无效数据');
                return;
            }

            var employeeData = res.record[0] || {};
    ...
    if (empIdField && empIdField.getValue()) {
        loadEmployeeInfo();
    }
