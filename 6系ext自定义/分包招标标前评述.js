function allReadyEdit() {
    ...
    dgrid.addListener('edit', function (editor, e) {
        ...
        if (e.field == 'u_spdf') {
            ...
            Ext.Array.each(dstore.data.items, function (record) {
                console.log("record.data.u_fbsmc_EXName:",record.data.u_fbsmc_EXName);
                if (record.get('u_spdf') > 69.5) {
                    record.set('u_pxjg', '通过')
                    sum1 += 1
                    if (record.get('u_fbszxdj') == 1) {
                        console.log('record:',record);
                        apass = apass + record.data.u_fbsmc_EXName + "\n" + "/";
                        a1 += 1
                    }
                    else if (record.get('u_fbszxdj') == 2) {
                        b1 += 1
                        bpass = bpass + record.data.u_fbsmc_EXName + "\n" + "/";
                    }
                    else if (record.get('u_fbszxdj') == 3) {
                        c1 += 1
                        cpass = cpass + record.data.u_fbsmc_EXName + "\n" + "/";
                    }
                }
                else {
                    ...
                    if (record.get('u_fbszxdj') == 1) {
                        a2 += 1
                        aupass = aupass + record.data.u_fbsmc_EXName + "\n" + "/";
                    }
                    else if (record.get('u_fbszxdj') == 2) {
                        b2 += 1
                        bupass = bupass + record.data.u_fbsmc_EXName + "\n" + "/";
                    }
                    else if (record.get('u_fbszxdj') == 3) {
                        c2 += 1
                        cupass = cupass + record.data.u_fbsmc_EXName + "\n" + "/";
                    }
                }
    ...
    dgrid1.getColumn('empid_EXName').getEditor().addListener('helpselected', function () {
        ...
        console.log('empidPhid:',empidPhid);
        execServer('p_form0000700224_dwbm', {
            ...
            if (data1[0]) {
                data[0].set('u_dwjbm', data1[0].extendObjects.xrzw);
            }
        });
    })
