function allReadyEdit() {
    ...
    if (otype == $Otype.VIEW) {
        ...
        Toolbar.items.get('changeStatus').on('click', function () {
            ...
            execServer('jsqxsq', {
                ...
                if (res.count > 0) {
                    ...
                    for (var i = 0; i < res.count; i++) {
                        if (data[i].extendObjects.roleno == 'FZAdmin') {
                            temp = 1;
                            break;
                        }
                    }
            ...
            if (Creator != $appinfo.userID || temp != 1) {
    ...
    if (otype == $Otype.ADD || otype == $Otype.EDIT) {
        ...
        mstform.getItem('deptid').addListener('beforetriggerclick', function () {
            var phid_org = mstform.getItem('phid_org').getValue();
            if (phid_org) {
                mstform.getItem('deptid').setOutFilter({
                    parent_orgid: phid_org,
                });
            }
        });
