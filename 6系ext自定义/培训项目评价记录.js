function allReadyEdit() { //初始化
    ...
    mstform.getItem('pxxmmc').addListener('beforetriggerclick', function () {
        ...
        if (Ext.isEmpty(phid_org) || Ext.isEmpty(pxnd)) {
            ...
            // });
            //p_form0000000088_m.ischeck='1' and p_form0000000084_d.phid not in(select pfm.pxxmmc from p_form0000000089_m pfm where pfm.pxxmmc is not null)
            mstform.getItem('pxxmmc').setClientSqlFilter(
                ...
                + " and p_form0000000084_d.phid not in(select pfm.pxxmmc from p_form0000000089_m pfm where pfm.pxxmmc is not null)"
            )
        }
    });
