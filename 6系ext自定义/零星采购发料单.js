function allReadyEdit() {
    ...
    var dgrid = Ext.getCmp('p_form0000700183_d');
    //增加按钮
    ...
    if (otype == $Otype.VIEW) {
        ...
        execServer('lxcgwzfldzb', {
            ...
            if (res.count > 0) {
                ...
                if (data1[0].extendObjects.ischeck == '1') {
                    Toolbar.insert(18, {
                        itemId: "push",
                        text: "推送浪潮",
                    ...
                    Toolbar.items.get('push').on('click', function () {
                        ...
                        Ext.Ajax.request({
                            ...
                            url: "http://172.20.65.5:30599/new_esey/lcApi/pushWZFLDLXCG?phid=" + busid,
                            async: false, //同步请求
    ...
    dgrid.getColumn('u_cllb_EXName').getEditor().addListener('beforetriggerclick', function () {
        console.log("mstform.getItem('u_ywlx'):", mstform.getItem('u_ywlx'));
        ...
        dgrid.getColumn('u_cllb_EXName').getEditor().setClientSqlFilter(" u_nm  in (  select YWLBCLLBSET_CLID from cllbdzbfld where  BZZXYWLB_NM in (select gs_nm from p_form0000000257_d where phid= ' " + user_ywlx + " ' )  )");
    });
    ...
    dgrid.getColumn('u_bm_EXName').getEditor().on('helpselected', function (eOp, ignoreBeforeEvent) {
        ...
        execServer('p_form0000700183_ngdept', {
            dept: u_bm
        }, function (res) {
            if (res.count > 0) {
                const data1 = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
        ...
        execServer('p_form0000700183_ngdeptlchsbm', {
            dept: u_bm
        }, function (res) {
            if (res.count > 0) {
                const data1 = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
                ...
                Ext.Msg.alert('提示', '请联系财务共享中心，部门对照不存在');
                data[0].set('u_lchsbmzjm', null);
                return false;
            }
        });
    ...
    mstform.getItem('u_lxcg_dept').addListener('helpselected', function () {
        ...
        execServer('ssxmb_bmywdy', {
            ...
            if (res.count > 0) {
                ...
                if (data1.length == 1) {
                    if (data1[0].extendObjects.user_lcywdy) {
                        mstform.getItem('u_lcywdy').setValue(data1[0].extendObjects.user_lcywdy);
                    } else {
                        Ext.Msg.alert('提示', '请联系财务共享中心，部门对照未做');
        ...
        execServer('ssxmb_zjm', {
            ...
            if (res.count > 0) {
                ...
                if (data1.length == 1) {
                    if (data1[0].extendObjects.user_mnemcodeinaccdepart) {
                        mstform.getItem('u_lchsbmzjm').setValue(data1[0].extendObjects.user_mnemcodeinaccdepart);
                    } else {
                        Ext.Msg.alert('提示', '请联系财务共享中心，部门对照未做');
    ...
    dgrid.addListener('edit', function (editor, e) {
        ...
        if (a == 0) {
            mstform.getItem('u_ckje').setValue('');
        }
        var u_ckje = 0; //出库金额
...
function beforeSaveEdit() {
    ...
    if (a == 0) {
        mstform.getItem('u_ckje').setValue('');
        Ext.Msg.alert('提示！', '请添加明细！,不然无法推送浪潮！');
        Toolbar.enable();
        return false;
    }
    return true;
