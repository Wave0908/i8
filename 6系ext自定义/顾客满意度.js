function allReadyEdit() { //顾客满意度&客户评价表
    ...
    var dgrid = Ext.getCmp('p_form0000000029_d'); //表体容器
    var dstore = dgrid.store; //表体数据
    var dgrid1 = Ext.getCmp('p_form0000000029_d1'); //表体容器
    var dstore1 = dgrid1.store;
    var dgrid2 = Ext.getCmp('p_form0000000029_d2'); //表体容器
    ...
    if (otype == $Otype.ADD || otype == $Otype.EDIT) {
        ...
        if (otype == $Otype.ADD) {
            execServer('p_form0000000029_pq1', {
                'zz': $appinfo.orgID
            }, function (res) {
                if (res.status != 'success') { //判断取数状态
                    ...
                    for (var i = 0; i < res.count; i++) {
                        arr.push({
                            u_pc_dept: data[i].extendObjects.u_pc_dept,
                            u_pc_dept_EXName: data[i].extendObjects.u_pc_dept_exname,
                            project_name: data[i].extendObjects.pcid,
                            project_name_EXName: data[i].extendObjects.project_name
                        });
                    }
            ...
            //停工项目填充
            execServer('p_form0000000029_pq2', {
                'zz': $appinfo.orgID
            }, function (res) {
                if (res.status != 'success') { //判断取数状态
                    Ext.Msg.alert('提示', '服务端取数失败');
                    ...
                    for (var i = 0; i < res.count; i++) {
                        arr.push({
                            u_pc_dept: data[i].extendObjects.u_pc_dept,
                            u_pc_dept_EXName: data[i].extendObjects.u_pc_dept_exname,
                            xmxx: data[i].extendObjects.xmxx,
                            xmxx_EXName: data[i].extendObjects.xmxx_exname
                        });
                    }
                    dstore2.insert(dstore1.getCount(), arr); //将服务端获取的数组内容插入到单据体
                }
            })
            //已完已结和已完未结项目填充
            execServer('p_form0000000029_pq3', {
                'zz': $appinfo.orgID
            }, function (res) {
                if (res.status != 'success') { //判断取数状态
                    Ext.Msg.alert('提示', '服务端取数失败');
                    ...
                    dstore1.insert(dstore2.getCount(), arr); //将服务端获取的数组内容插入到单据体
                }
            })
        ...
        window.onload = function () {
            ...
            mstform.getItem('zzffzs').addListener('change', function () {
                var num = dstore.getCount() || 0;
                var b = Number(mstform.getItem('zzffzs').getValue()) || 0; //发放张数
                
                // 检查并计算比率
                if (num > 0 && b >= 0) {
                    var ratio = (b / num).toFixed(4);
                    mstform.getItem('zjhfl').setValue(ratio);
                    console.log('计算回访率：', {数据总数: num, 发放张数: b, 回访率: ratio});
                } else {
                    mstform.getItem('zjhfl').setValue(0);
                    console.log('计算回访率：数据异常', {数据总数: num, 发放张数: b});
                }
            });
            mstform.getItem('zzshzs').addListener('change', function () {
                ...
                if (a > b && a && b) {
                    ...
                    mstform.getItem('zzshzs').setValue(null);
                } else {
            ...
            mstform.getItem('wbffzs').addListener('change', function () {
                var num = dstore1.getCount() || 0;
                var b = Number(mstform.getItem('wbffzs').getValue()) || 0; //发放张数
                var a = Number(mstform.getItem('wbshzs').getValue()) || 0; //收回张数
                
                // 检查并计算发放率
                if (num > 0 && b >= 0) {
                    var ratio = (b / num).toFixed(4);
                    mstform.getItem('wbhfl').setValue(ratio);
                    console.log('计算维保回访发放率：', {数据总数: num, 发放张数: b, 发放率: ratio});
                } else {
                    mstform.getItem('wbhfl').setValue(0);
                    console.log('计算维保回访发放率：数据异常', {数据总数: num, 发放张数: b});
                }
                
                // 检查并计算回收率
                if (b > 0 && a >= 0) {
                    var recoveryRatio = (a / b).toFixed(4);
                    mstform.getItem('wbhsl').setValue(recoveryRatio);
                    console.log('计算维保回访回收率：', {发放张数: b, 收回张数: a, 回收率: recoveryRatio});
                } else {
                    mstform.getItem('wbhsl').setValue(0);
                    console.log('计算维保回访回收率：数据异常', {发放张数: b, 收回张数: a});
                }
            });
            mstform.getItem('wbshzs').addListener('change', function () {
                ...
                if (a > b && a && b) {
                    Ext.Msg.alert('提示', '收回张数大于发放张数，请修改');
                    mstform.getItem('wbshzs').setValue(null);
                } else {
            ...
            mstform.getItem('tgfszs').addListener('change', function () {
                var num = dstore2.getCount() || 0;
                var b = Number(mstform.getItem('tgfszs').getValue()) || 0; //发放张数
                
                // 检查并计算比率
                if (num > 0 && b >= 0) {
                    var ratio = (b / num).toFixed(4);
                    mstform.getItem('tghfl').setValue(ratio);
                    console.log('计算停工项目回访率：', {数据总数: num, 发放张数: b, 回访率: ratio});
                } else {
                    mstform.getItem('tghfl').setValue(0);
                    console.log('计算停工项目回访率：数据异常', {数据总数: num, 发放张数: b});
                }
            });
            mstform.getItem('tgshzs').addListener('change', function () {
                ...
                if (a > b && a && b) {
                    Ext.Msg.alert('提示', '收回张数大于发放张数，请修改');
                    mstform.getItem('tgshzs').setValue(null);
                } else {
...
function beforeSaveEdit(type) {
    ...
    if (otype == $Otype.ADD) {
        ...
        for (var i = 0; i < dstore.getCount(); i++) {
            ...
            //console.log(id);
            execServer('p_form0000000029_save_jc', {
                'y': y,
                'jd': jd
            }, function (res) {
                ...
                if (res.count > 0) {
                    for (var j = 0; j < res.count; j++) {
                        ...
                        var data = JSON.parse(res.data);
                        if (data[j].extendObjects.project_name == pc_name) {
                            console.log(data[j].extendObjects.project_name);
        ...
        for (var i = 0; i < dstore2.getCount(); i++) {
            var pc = dstore2.getAt(i).get('xmxx')
            //var nm = dstore.getAt(i).get('empid')
            //console.log(id);
            execServer('p_form0000000029_save_jc', {
                'y': y,
                ...
                if (res.count > 0) {
                    for (var j = 0; j < res.count; j++) {
                        ...
                        if (data[j].extendObjects.xmxx == pc) {
                            console.log(data[j].extendObjects.xmxx);
            ...
            execServer('p_form0000000029_save_jc', {
                ...
                if (res.count > 0) {
                    for (var j = 0; j < res.count; j++) {
                        //console.log(res.record[i].year);
                        const data = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
                        if (data[j].extendObjects.xmxx == pc_name) {
                            console.log(data[j].extendObjects.xmxx);
