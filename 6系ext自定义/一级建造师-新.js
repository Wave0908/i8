...
function allReadyEdit() {
    ...
    var d3grid = Ext.getCmp('p_form0000700747_d3');
    console.log("d3grid:", d3grid);
    var d3store = d3grid.store;
    var d4grid = Ext.getCmp('p_form0000700747_d4');
    ...
    */
    ...
    mstform.getItem('empid').addListener('helpselected', function (obj) {
        ...
        execServer('getryxx', {
            ...
            if (res.count > 0) {
                ...
                if (data1.length > 0) {
                    mstform.getItem("u_xb").setValue(data1[0].extendObjects.sex); //性别
                    mstform.getItem("u_sr").setValue(data1[0].extendObjects.birthday); //生日
                    mstform.getItem("u_xl").setValue(data1[0].extendObjects.zgxl); //最高学历
                    mstform.getItem("u_jg").setValue(data1[0].extendObjects.szbm); //所在部门、职能机构
                    mstform.getItem("u_gw").setValue(data1[0].extendObjects.user_xrzw); //岗位、人员职务
                    mstform.getItem("u_zc").setValue(data1[0].extendObjects.zc); //职称
                    mstform.getItem("u_zz").setValue(data1[0].extendObjects.empstatus);//员工状态、在职状态
                    mstform.getItem("u_yglx").setValue(data1[0].extendObjects.htgx); //合同关系、员工类型
                    //mstform.getItem("u_zsbh").setValue(data1[0].extendObjects.cno); //合同关系、员工类型
                    mstform.getItem("bill_name").setValue(data1[0].extendObjects.cname); //标题
                }
    ...
    Toolbar.get('myadd').on('click', function () {
        //判断项目信息是否选择 
        if (Ext.isEmpty(mstform.getItem("empid").getValue())) {
            //未选择提示：请先选择人员！
    ...
    */
    d4grid.getColumn('u_gcxm_EXName').getEditor().addListener('helpselected', function () {
        var data = d4grid.getSelectionModel().getSelection();
        console.log("getSelectionModel data:", data);
        console.log(" data[0].data.u_gcxm:", data[0].data.u_gcxm)
        var phid = data[0].data.u_gcxm;
        execServer('p_form0000700747_yjxx', {
            'phid': phid
        }, function (res) {
            console.log(res);
            if (res.count > 0) {
                const data2 = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
                data[0].set('textcol_3', data2[0].extendObjects.qdlx);
                data[0].set('datetimecol_2', data2[0].extendObjects.htqdsj);
            }
        });
    })
}