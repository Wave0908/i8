function allReadyEdit() {
    ...
    mstform.getItem('phid_pc').on('beforetriggerclick', function (eOp, ignoreBeforeEvent) {
        ...
        /*PC对应通用帮助拼接SQL语句条件过滤 ocode    *arr的用法可能不对，这里是SQL语句的拼接 arr数组无法In识别*/
    ...
    mstform.getItem('phid_pc').addListener('helpselected', function (e) {
        console.log("选择帮助phid_pc");
        mstform.getItem('u_xhh').setValue(e.data.PcNo);

        var pc = e.data.PhId
        console.log("e.data.PhId pc:", pc);
        execServer('p_form0000700078_xm', { 'pc': pc }, function (res) {
            console.log(res);
            if (res.count > 0) {
                const data = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
                console.log("data:", data);
                var record = data[0].extendObjects.phid_pc;
                ...
                console.log("pc:", pc);
                // 对 项目类型进行判断
                execServer('p_form0000700078_xmmc', { 'pc': pc }, function (res) {
                    if (res.count > 0) {
                        const data = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
                        var projectname = data[0].extendObjects.projectname;
                        mstform.getItem('bill_name').setValue(forDate(dt) + projectname);
                    }
    ...
    function chlx(e) {
        if (e == '974191216000024') {
            ...
            mstform.getItem('numericcol_16').setVisible(false);
            mstform.getItem('numericcol_17').setVisible(false);
            mstform.getItem('numericcol_18').setVisible(false);
            mstform.getItem('numericcol_19').setVisible(false);
            mstform.getItem('numericcol_20').setVisible(false);
