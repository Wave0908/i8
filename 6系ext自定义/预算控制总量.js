function allReadyEdit() { //页面编辑方法，固定写法
    ...
    //获取表体容器
    var dgrid = Ext.getCmp('p_form0000700456_d');
    ...
    arr1.push({
        ...
        u_yskkzlwlkz_EXName: '01钢结构类',
        u_jldw: '吨',
    });
    arr1.push({
        u_yskkzlwlkz: '569000000001007',
        u_yskkzlwlkz_EXName: '01螺线类',
        u_jldw: '吨',
    });
    arr1.push({
        u_yskkzlwlkz: '569000000001008',
        u_yskkzlwlkz_EXName: '07混凝土类1',
        u_jldw: '立方米',
    });
    arr1.push({
        u_yskkzlwlkz: '569000000001031',
        u_yskkzlwlkz_EXName: '07混凝土类2',
        u_jldw: '吨',
    });
    arr1.push({
        u_yskkzlwlkz: '569000000001009',
        u_yskkzlwlkz_EXName: '26电缆类',
        u_jldw: '米',
    });
    ...
    if (otype == $Otype.VIEW) {
        ...
        setTimeout(function () {
            ...
            if (bglx == 1) {
                execServer('p_form0000700456_sfczbg_zc', { 'bill_no': bill_no }, function (res) {
                    if (res.count > 0) {
                        const data1 = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
                        var bgd = data1[0].extendObjects.bill_no
                        Ext.Msg.alert('提示', '该预算控制单已存在变更记录[' + bgd + ']，已隐藏“申请去审”按钮，请去审删除对应单据后再操作');
                        Toolbar.get('check').hide();
                    }
                });
            }
            if (bglx == 2) {
                var zc = mstform.getItem('u_yskzdh').getValue();
                execServer('p_form0000700456_sfczbg_bg', { 'zc': zc }, function (res) {
                    if (res.count > 0) {
                        const data1 = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
                        if (bill_no != data1[0].extendObjects.bill_no) {
                            var bgd = data1[0].extendObjects.bill_no
    ...
    mstform.getItem('phid_pc').on('beforetriggerclick', function (obj) {
        if (Ext.isEmpty(mstform.getItem('u_yskzbglx').getValue())) {
            Ext.Msg.alert('提示', '请先维护预算控制变更类型');
            return false;
        }
        if (mstform.getItem('u_yskzbglx').getValue() == '1') {
            mstform.getItem('phid_pc').setClientSqlFilter(" project_table.phid  not in ( select phid_pc from p_form0000700456_m where u_yskzbglx='1'  ) ");
        }
        if (mstform.getItem('u_yskzbglx').getValue() == '2') {
            mstform.getItem('phid_pc').setClientSqlFilter(" project_table.phid  not in ( select phid_pc from p_form0000700456_m where u_yskzbglx='2' and  app_status <> '1' ) ");
        }
    ...
    mstform.getItem('phid_pc').on('helpselected', function (obj) {
        mstform.getItem('u_yskzdh').setValue();
    ...
    mstform.getItem('u_yskzdh').on('beforetriggerclick', function (obj) {
        ...
        mstform.getItem('u_yskzdh').setClientSqlFilter(" phid  not in ( select u_yskzdh from p_form0000700456_m where u_yskzbglx='2' and  app_status <> '1' ) and p_form0000700456_m.phid_pc = ' " + mstform.getItem('phid_pc').getValue() + "'");
    ...
    mstform.getItem('u_yskzdh').on('helpselected', function (obj) {
        ...
        execServer('p_form0000700456_yskzlmx', { 'phid': u_yskzdh }, function (res) {
            var a = dgrid.getStore().getRange(0, dstore.getCount() - 1);
            const data1 = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
            ...
            for (var i = 0; i < a.length; i++) {
                if (a[i].get('u_yskkzlwlkz') == '569000000001006') {
                    for (var j = 0; j < data1.length; j++) {
                        if (data1[j].extendObjects.u_yskkzlwlkz == '569000000001006') {
                            a[i].set('qty', data1[j].extendObjects.qty);
                            a[i].set('amt', data1[j].extendObjects.amt);
                            a[i].set('u_bghsl', data1[j].extendObjects.qty);
                            a[i].set('u_bghhsje', data1[j].extendObjects.amt);
                        }
                    }
                ...
                if (a[i].get('u_yskkzlwlkz') == '569000000001007') {
                    for (var j = 0; j < data1.length; j++) {
                        if (data1[j].extendObjects.u_yskkzlwlkz == '569000000001007') {
                            a[i].set('qty', data1[j].extendObjects.qty);
                            a[i].set('amt', data1[j].extendObjects.amt);
                            a[i].set('u_bghsl', data1[j].extendObjects.qty);
                            a[i].set('u_bghhsje', data1[j].extendObjects.amt);
                ...
                if (a[i].get('u_yskkzlwlkz') == '569000000001008') {
                    for (var j = 0; j < data1.length; j++) {
                        if (data1[j].extendObjects.u_yskkzlwlkz == '569000000001008') {
                            a[i].set('qty', data1[j].extendObjects.qty);
                            a[i].set('amt', data1[j].extendObjects.amt);
                            a[i].set('u_bghsl', data1[j].extendObjects.qty);
                            a[i].set('u_bghhsje', data1[j].extendObjects.amt);
                ...
                if (a[i].get('u_yskkzlwlkz') == '569000000001031') {
                    for (var j = 0; j < data1.length; j++) {
                        if (data1[j].extendObjects.u_yskkzlwlkz == '569000000001031') {
                            a[i].set('qty',data1[j].extendObjects.qty);
                            a[i].set('amt', data1[j].extendObjects.amt);
                            a[i].set('u_bghsl', data1[j].extendObjects.qty);
                            a[i].set('u_bghhsje', data1[j].extendObjects.amt);
                ...
                if (a[i].get('u_yskkzlwlkz') == '569000000001009') {
                    for (var j = 0; j < data1.length; j++) {
                        if (data1[j].extendObjects.u_yskkzlwlkz == '569000000001009') {
                            a[i].set('qty', data1[j].extendObjects.qty);
                            a[i].set('amt', data1[j].extendObjects.amt);
                            a[i].set('u_bghsl', data1[j].extendObjects.qty);
                            a[i].set('u_bghhsje', data1[j].extendObjects.amt);
    ...
    if (Toolbar.get('check')) {
        ///增加去审判断，存在后续单据，不可去审删除
        Toolbar.get('check').on('click', function () {
            var bill_no = mstform.getItem('bill_no').getValue();
            var bglx = mstform.getItem('u_yskzbglx').getValue();
            var flag = 0
            if (bglx == 1) {
...
function beforeSaveEdit() {
    ...
    var flag = 0
    if (lx == '1') {
        execServer('p_form0000700456_pdzc', { 'phid_pc': xm }, function (res) {
            if (res.count > 0) {
                var data = JSON.parse(res.data)
                var kzzl = data[0].bill_no
                Ext.MessageBox.show({
                    title: '提示',
