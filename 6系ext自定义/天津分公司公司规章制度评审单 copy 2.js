function allReadyEdit() {
    ...
    mstform.on('dataready', function (e) {
        if (otype == $Otype.ADD) {
            var test = mstform.getItem('u_xdqmc').userSetMustInput(false);
    ...
    function mustInput(value, attribute) {
        ...
        for (var i = 0; i < value.length; i++) {
            ...
            var item = mstform.getItem(value[i]);
            if (item) {
                item.userSetMustInput(attribute);
            } else {
                console.warn("Field not found in mstform: " + value[i]);
            }
        }
    }
    ...
    function readOnly(value, attribute) {
        ...
        for (var i = 0; i < value.length; i++) {
            var item = mstform.getItem(value[i]);
            if (item) {
                item.userSetReadOnly(attribute);
            } else {
                console.warn("Field not found in mstform: " + value[i]);
            }
        }
    ...
    function xdzt(st1, st2) {
        ...
        var value2 = ['u_xdqmc', 'checkboxcol_4', 'checkboxcol_5', 'checkboxcol_6', 'checkboxcol_7', 'checkboxcol_8', 'u_ygzzdwh', 'u_yrq', 'u_xdzynr'];
        var att2 = st2;
        readOnly(value2, att2);

        var att1 = '';
        setValue(value1, att1);
        setValue(value2, att1);
    }
    ...
    function fzzt(st1, st2) {
        ...
        var value2 = ['checkboxcol_9', 'checkboxcol_10', 'checkboxcol_11', 'checkboxcol_12', 'checkboxcol_13', 'u_ygzzdwh2', 'u_ygzrq', 'u_fzyy'];
        var att2 = st2;
        readOnly(value2, att2);
