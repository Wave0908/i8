        ...
        Toolbar.get('YY').on('click', function () { //按钮更新事件
            ...
            if (lb == 1) {
                ...
                var help = Ext.create('Ext.ng.RichHelp', {
                    ...
                    outFilter: {
                        'zckp_ls.phid_org': mstform.getItem('phid_org').getValue()
                    }
                });
                help.showHelp();
                help.on('helpselected', async function (obj) {
                    mstform.getItem('zcmc').setValue(obj.data.zcmc);
                    mstform.getItem('bill_name').setValue(obj.data.zcmc);
                    //来源管理 glbm1 使用glbm
                    ...
                    await execServer('p_form0000000225_zc_glzz', {
                        'A': glzz
                    }, function (res) {
                        if (res.count > 0) {
                            const data = JSON.parse(res.data);
                            mstform.getItem('phid_org').setValue(data[0].extendObjects.phid);
                        }
                    });
                    await execServer('p_form0000000225_zc_glzz', {
                        'A': syzz
                        ...
                        if (res.count > 0) {
                            const data = JSON.parse(res.data);
                            console.log('p_form0000000225_zc_glzz data=============>', data)
                            mstform.getItem('u_code').setValue(data[0].extendObjects.phid);
                        }
                    });
                    //代码转名称
                    BatchBindCombox([
                        ...
                        mstform.getItem('phid_org'),
                        // TODO user_ocode有问题
                        mstform.getItem('u_code'),
            ...
            if (lb == 2) {
                ...
                var help = Ext.create('Ext.ng.RichHelp', {
                    ...
                    //过滤条件，m_code为过滤字段，userhelp_1为过滤字段的值
                    outFilter: {
                        'zckp_fc.phid_org': mstform.getItem('phid_org').getValue()
                    }
                });
                help.showHelp();
                //表头赋值
            ...
            if (lb == 3) {
                ...
                var help = Ext.create('Ext.ng.RichHelp', {
                    ...
                    //过滤条件，m_code为过滤字段，userhelp_1为过滤字段的值
                    outFilter: {
                        'zckp_sb.ocode': mstform.getItem('phid_org').getValue()
                    }
                });
                help.showHelp();
                help.on('helpselected', async function (obj) {
            ...
            if (lb == 4) {
                ...
                var help = Ext.create('Ext.ng.RichHelp', {
                    ...
                    //过滤条件，m_code为过滤字段，userhelp_1为过滤字段的值
                    outFilter: {
                        'zckp_wx.phid_org': mstform.getItem('phid_org').getValue()
                    }
                });
                help.showHelp();
                help.on('helpselected', async function (obj) {
