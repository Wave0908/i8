function allReadyEdit() {
    ...
    mstform.getItem('custno').addListener('helpselected', function () {
        ...
        callServer('khxx', [{
            ...
            if (!Ext.isEmpty(res)) {
                mstform.getItem("u_cym").setValue(res.record[0].oldname); //曾用名
                mstform.getItem("u_gj").setValue(res.record[0].country); //国家
                mstform.getItem("u_sf").setValue(res.record[0].province); //区域
                mstform.getItem("u_cs").setValue(res.record[0].city); //是否一带一路
                mstform.getItem("u_qx").setValue(res.record[0].district); //行业类别
            }
        });
    ...
    mstform.getItem('u_khlxej').on('beforetriggerclick', function () { //客户类型2
        if (mstform.getItem('u_khlxyj').getValue() == '') {
    ...
    dgrid.getColumn('u_khywglej_name').getEditor().on('beforetriggerclick', function (eOp, ignoreBeforeEvent) {
    ...
    dgrid.getColumn('u_khywglej_name').getEditor().on('beforetriggerclick', function (eOp, ignoreBeforeEvent) {
        ...
        callServer('zcfl', [{
            ...
            fl = res.record[0].yjbm
        });
        dgrid.getColumn('u_khywglej_name').getEditor().setOutFilter({
            'yjbm': fl
        });
  ...
  */
    ...
    d3grid.getColumn('u_hylx_name').getEditor().on('beforetriggerclick', function (eOp, ignoreBeforeEvent) {
        ...
        callServer('zzzlxh', [{
            ...
            fl = res.record[0].资质种类序号
        });
        d3grid.getColumn('u_hylx_name').getEditor().setOutFilter({
            '资质种类序号': fl
        });
    });
    ...
    if (otype == $Otype.ADD) {
        ...
        mstform.getItem('title').addListener('change', function () {
            var bt = mstform.getItem('u_bt');
            var xmmc_val = "'".concat(mstform.getItem("title").getValue()).concat("'");
            callServer('List', [{
                'title': xmmc_val
            }], function (res) {
                mstform.getItem('u_bt').setValue(res.record[0].u_bt);
            });
            if (mstform.getItem("u_bt").getValue() > 0) {
                Ext.Msg.alert('提示', '此客户已录入客户详细信息!');
                mstform.getItem('title').focus();
