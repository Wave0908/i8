function allReadyEdit() {
    ...
    if (otype == $Otype.VIEW) {
        mstform.on('dataready', function (e) {
            if (Toolbar.getComponent('applycheck')) {
                ...
                if (dbfs == 1) {
                    ...
                    //     var zcbm = a[i].get('zcbm');
                    //     console.log("zcbm:", zcbm);
                    // }
                    //dbtzdczcjtzjbm
                    if (bill_dt && bill_no && phid_org) {
                        console.log("bill_dt:", bill_dt);
                        console.log("bill_no:", bill_no);
                        console.log("phid_org:", phid_org);
                        execServer('dbtzdczcjtzjbm', {
                            'dt': "'" + bill_dt + "'",
                            'billno': "'" + bill_no + "'",
                            'phid': "'" + phid_org + "'"
                        }, function (res) {
        ...
        // if (Toolbar.items.get('applycheck')) {
        //     Toolbar.items.get('applycheck').on('click', function () {
    ...
    Toolbar.items.get('push').on('click', function () {
        Ext.Ajax.request({
            ...
            url: "http://172.20.65.5:30599/new_esey/lcApi/pushDbtzDrOrDc?phid=" + busid,
            async: false, //同步请求
    ...
    if (otype == $Otype.ADD || otype == $Otype.EDIT) { //var a = document.getElementById('zcxz-inputEl').set
        ...
        execServer('p_form0000000236_zz', {
            ...
            mstform.getItem('bill_name').setValue(data[0].extendObjects.deptname + '资产调拨单');
        });
        //浪潮核算部门过滤
        ...
        mstform.getItem('deptid1').on('beforetriggerclick', function (eOp, ignoreBeforeEvent) { //帮助窗口打开前事件
            var zz = mstform.getItem('phid_org').getValue();
            mstform.getItem('deptid1').setOutFilter({
                parent_orgid: zz
            }) //PC对应通用帮助中的字段 pc_no、creator进行条件过滤
        });
        mstform.getItem('deptid').on('beforetriggerclick', function (eOp, ignoreBeforeEvent) { //帮助窗口打开前事件
            var zz = mstform.getItem('phid_org').getValue();
            mstform.getItem('deptid').setOutFilter({
                parent_orgid: zz
            }) //PC对应通用帮助中的字段 pc_no、creator进行条件过滤
        });
        //表体控制
            ...
            if (mstform.getItem('dbfs').getValue() == 2) {
                ...
                var help = Ext.create('Ext.ng.MultiRichHelp', { //创建通用帮助多选
                    ...
                    outFilter: {
                        ...
                        'yy_zckp.phid_org': mstform.getItem('phid_org').getValue()
                    } //帮助对应过滤条件，帮助过滤字段等于表单字段  
                }); //创建通用帮助多选事件结束 ｝
                help.showHelp();
                help.on('helpselected', function (obj) { //通用帮助选择更新
