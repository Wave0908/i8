function allReadyEdit() { //初始化
    ...
    var dgrid = Ext.getCmp('p_form0000000096_dgrid');
    var dgrid1 = Ext.getCmp('p_form0000000096_d4grid');
    ...
    mstform.getItem('train_project').addListener('beforetriggerclick', function() {
        ...
        if(Ext.isEmpty(pxnd) || Ext.isEmpty(ocode)) {
            Ext.Msg.alert('提示', '请先选择培训年度和组织！');
            return false;
        } else {
            mstform.getItem('train_project').setOutFilter({
                'p_form0000000084_m.ocode': mstform.getItem('ocode').getValue(),
...
function getSaveDataList(type) {
    var mstform = Ext.getCmp('p_form0000000096_mgrid');
    var data = mstform.getSelectionModel().getSelection();
    var phid = Ext.Number.from(data[0].get('train_project'), 0);
    if(type == 'Delete') {
        return {
            funcname: 'crs',
            paramstr: [{
