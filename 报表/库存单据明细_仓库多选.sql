SELECT
  ...
  kc_billhead.bill_no 单据号,
  project_table.bill_name 项目,
  fg3_enterprise.compname 供应商,
  pcm3_cnt_m.bill_no 合同编号,
  pcm3_cnt_m.bill_name 合同名称,
  warehouse.whname 仓库,
...
FROM
  ...
  LEFT JOIN project_table ON project_table.phid = kc_billhead.phid_pc
  LEFT JOIN warehouse ON warehouse.phid = kc_billhead.phid_warehouse
  LEFT JOIN msunit ON msunit.phid = res_master.phid_unit
  LEFT JOIN pcm3_cnt_m ON kc_billhead.phid_contractno = pcm3_cnt_m.phid
  LEFT JOIN fg3_enterprise ON kc_billhead.phid_supply_ent = fg3_enterprise.phid
WHERE
  kc_billhead.bill_dt >= @TransDtBgn
  AND kc_billhead.bill_dt <= @TransDtEnd
  AND COALESCE (kc_billbody.split_type, 0) <> 1
ORDER BY
