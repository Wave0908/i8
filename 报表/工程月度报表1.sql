SELECT
  ...
  CAST(COALESCE(gj.nat_name,'') || COALESCE(sf.cname,'') || COALESCE(ct.cname,'') AS VARCHAR) as 项目所在地区,
  ...
  CASE
    ...
    WHEN pt.user_cjms = '5' THEN
      '其它'
  END AS 承建模式,
  to_char(pt.start_date, 'YYYY-MM-DD') AS 合同开工日期,
  to_char(pt.end_date, 'YYYY-MM-DD') 合同竣工日期,
  to_char(pt.limit_time, 'YYYY-MM-DD') 合同工期_天,
  user_yztzrq 业主调整日期含签证,
  to_char(pt.user_tzhjgrq, 'YYYY-MM-DD') 调整后竣工日期,
  user_tzhzgq  调整后总工期,
  pt.approx_contract_fc / 10000 合同额_万元,
  glry.glry 管理人员人数,
  zyry.zyry 作业人员人数,
  CAST(zt.descript AS varchar) 项目状态,
  to_char(ghdg.app_dt, 'YYYY-MM-DD') 项目管理规划大纲审批日期,
  to_char(xmbcl.app_dt, 'YYYY-MM-DD') 项目部成立日期,
  to_char(zrs.app_dt, 'YYYY-MM-DD') 项目管理目标责任书签订日期,
  to_char(ssghsp.app_dt, 'YYYY-MM-DD') 项目管理实施规划审批日期,
  ...
  to_char(kg.reo_scdt, 'YYYY-MM-DD') 内部开工审批日期,
  to_char(kg.kgbg_dt, 'YYYY-MM-DD') 外部开工报告审批日期,
  to_char(pf545_m.u_sjkgrq, 'YYYY-MM-DD') 项目实际开工日期,
  to_char(pt.user_jhjgrq, 'YYYY-MM-DD') 计划竣工日期,
  to_char(pt.user_wgrq + INTERVAL '3 month', 'YYYY-MM-DD') 预计竣工日期,
  ...
  to_char(tg.gh_dt, 'YYYY-MM-DD') 停工开始日期,
  to_char(tg.user_jhfg, 'YYYY-MM-DD') 计划复工日期,
  to_char(tg.user_sjfgrq, 'YYYY-MM-DD') 实际复工日期,
  ljtg.累计停工 累计实际停工天数,
  CAST(tg.user_tyyy AS varchar) 停工原因,
  CAST(tg.tg_why AS varchar) 原因简要描述,
  to_char(pt.user_wgrq, 'YYYY-MM-DD') 实体完工日期,
  CAST(fg.oname AS varchar) 责任单位,
  to_char(pt.app_dt, 'YYYY-MM-DD') AS 审批日期,
  CASE
  ...
  CASE
    WHEN to_char(pt.app_dt, 'YYYY') || right('00' || to_char(pt.app_dt, 'MM'), 2) = @uyear || @mon THEN
      '是'
    ELSE
      '否'
  END AS 是否为本月新增项目,
...
FROM
  ...
  AND tg.reo_type = 2
  AND tg.app_status = 1
  AND to_char(tg.gh_dt, 'YYYYMM') <= @uyear || @mon
  AND to_char(tg.user_jhfg, 'YYYYMM') >= @uyear || @mon
  --停工
  LEFT JOIN (SELECT
      tg.phid_pc AS pc,
      SUM(
        (CASE
          WHEN to_char(tg.user_jhfg, 'YYYY-MM') <= to_char((CAST(@uyear || @mon || '01' AS date) + INTERVAL '1 month' - INTERVAL '1 day'), 'YYYY-MM') THEN
            tg.user_jhfg
          ELSE
            (CAST(@uyear || @mon || '01' AS date) + INTERVAL '1 month' - INTERVAL '1 day')
        END - tg.gh_dt) + 1
      ) 累计停工
    FROM
      reort_t_m tg
  ...
  AND jdqk.uyear = @uyear
  AND jdqk.mon = @mon --进度情况
  ...
  AND EXTRACT(YEAR FROM CAST(zyry.u_sbyf AS date)) = CAST(@uyear AS integer)
  AND EXTRACT(MONTH FROM CAST(zyry.u_sbyf AS date)) = CAST(@mon AS integer) --作业人员
  ...
  AND EXTRACT(YEAR FROM CAST(bdt AS date)) = CAST(@uyear AS integer)
  AND EXTRACT(MONTH FROM CAST(bdt AS date)) = CAST(@mon AS integer)
  --left join(
...
WHERE
  ...
  AND to_char(pt.app_dt, 'YYYYMM') <= @uyear || @mon
  AND sx.phid = '4'
