...
GROUP BY 
  ...
  LEFT JOIN (SELECT 所属分公司, NVL (SUM(回款金额), 0) hkje,项目类型 FROM jshkb WHERE 项目类型 = '智能建造-硬件' GROUP BY 所属分公司,项目类型) hk ON hk.所属分公司 = cwzz.oname
  LEFT JOIN (SELECT 所属分公司, NVL (SUM(剩余回款金额), 0) syhkje,项目类型 FROM jshkb WHERE 项目类型 = '智能建造-硬件' GROUP BY 所属分公司,项目类型) syhk ON syhk.所属分公司 = cwzz.oname
  LEFT JOIN (
    SELECT
      fo.oname AS 所属分公司,
      xpb.type_name AS 项目类型,
  ...
  LEFT JOIN (SELECT 所属分公司, NVL (SUM(已确认产值), 0) yqrcz,项目类型 FROM xmglb  WHERE 项目类型 = '智能建造-硬件' GROUP BY 所属分公司,项目类型) cz ON cz.所属分公司 = cwzz.oname
  left join (SELECT 所属分公司,COUNT(所属分公司) as xmsl,项目类型 FROM cbjdb WHERE 进度系数 > 1 and 项目类型 = '智能建造-硬件' group by 所属分公司,项目类型) cbjd ON cbjd.所属分公司 = cwzz.oname
  left join  (SELECT 所属分公司,COUNT(所属分公司) as xmsl,项目类型 FROM cbjdb WHERE 成本系数 > 1 and 项目类型 = '智能建造-硬件' group by 所属分公司,项目类型) cbjd1 ON cbjd1.所属分公司 = cwzz.oname
where zjxmsl.项目类型 = '智能建造-硬件'
