...
metrics_pool AS (
  ...
  FROM base_org o
  LEFT JOIN xmglb_raw x ON x.所属分公司 = o.oname
...
summarized_all AS (
  ...
  FROM metrics_pool GROUP BY dqname, oname
  UNION ALL
  -- 智能建造
  SELECT 
  ...
  FROM metrics_pool WHERE 原始类型 IN ('智能建造-硬件', '智能建造-软件') GROUP BY dqname, oname
  UNION ALL
  -- 信息化
  SELECT 
  ...
  FROM metrics_pool WHERE 原始类型 NOT IN ('智能建造-硬件', '智能建造-软件') OR 原始类型 IS NULL GROUP BY dqname, oname
)
-- 10. 最终输出
SELECT 
  dqname AS 所属大区,
  ...
  czsl AS 超预算项目数量
FROM summarized_all
WHERE (zjxmsl + jhyssl + bjsjysxmsl + fxyjsl + htzje + xmje + yqrcz) > 0
