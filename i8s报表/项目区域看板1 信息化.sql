...
xmglb_agg AS (
  ...
  FROM
    xmglb
  WHERE
    项目类型 NOT IN ('智能建造-硬件', '智能建造-软件')
  GROUP BY
    所属分公司
),
-- 3. 结算回款表汇总 (合同总额、回款、剩余回款)
jshkb_agg AS (
  SELECT
    所属分公司,
...
mb_agg AS (
  ...
  WHERE
    xpb.type_name NOT IN ('智能建造-硬件', '智能建造-软件')
  GROUP BY
    fo.oname
),
-- 5. 实际验收汇总
...
FROM
  ...
  LEFT JOIN jshkb_agg j ON j.所属分公司 = o.oname
  LEFT JOIN mb_agg m ON m.所属分公司 = o.oname
  LEFT JOIN accept_agg a ON a.所属分公司 = o.oname
  LEFT JOIN fx_agg f ON f.分公司 = o.oname
  LEFT JOIN cb_agg c ON c.所属分公司 = o.oname
ORDER BY
  o.dqname, o.oname
