...
xmglb_agg AS (
  SELECT
    所属分公司,
    CASE 
      WHEN 项目类型 = '智能建造-软件' THEN '智能建造-软件'
      WHEN 项目类型 = '智能建造-硬件' THEN '智能建造-硬件'
      ELSE '信息化' 
    END AS 项目分类,
...
mb_agg AS (
  SELECT
    fo.oname AS 所属分公司,
    CASE 
      WHEN xpb.type_name = '智能建造-软件' THEN '智能建造-软件'
      WHEN xpb.type_name = '智能建造-硬件' THEN '智能建造-硬件'
      ELSE '信息化' 
    END AS 项目分类,
...
final_combined AS (
  ...
  FROM
    ...
    LEFT JOIN jshkb_agg j ON j.所属分公司 = o.oname AND j.项目分类 = x.项目分类
    LEFT JOIN mb_agg m ON m.所属分公司 = o.oname AND m.项目分类 = x.项目分类
    LEFT JOIN accept_agg a ON a.所属分公司 = o.oname AND a.项目分类 = x.项目分类
    LEFT JOIN fx_agg f ON f.分公司 = o.oname AND f.项目分类 = x.项目分类
    LEFT JOIN cb_agg c ON c.所属分公司 = o.oname AND c.项目分类 = x.项目分类
)
-- 9. 最终输出 (信息化排在前面)
SELECT * FROM (
  SELECT * FROM final_combined WHERE 项目分类 = '信息化'
  UNION ALL
  SELECT * FROM final_combined WHERE 项目分类 = '智能建造-软件'
  UNION ALL
  SELECT * FROM final_combined WHERE 项目分类 = '智能建造-硬件'
) t
ORDER BY t.所属大区, t.所属分公司, t.项目分类
