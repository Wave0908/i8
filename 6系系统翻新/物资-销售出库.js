function AllReady() {
	...
	grid.queryById('addrow').on('click', function () {
		...
		console.log("generateId(store.getCount() + 1).toString()", generateId(store.getCount() + 1).toString())
					...
					if (res.count > 0) {
						...
						if (data.length == 1) {
							if (data[0].extendObjects.user_mnemcodeinaccdepart) {
								mstform.getItem('user_mnemcodeinaccdepart').setValue(data[0]
									.extendObjects.user_mnemcodeinaccdepart);
							} else {
