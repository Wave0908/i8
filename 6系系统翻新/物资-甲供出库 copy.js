function AllReady() {
    ...
    /*子表的使用部位通用帮助选择前start*/
    dgrid.getColumn('user_sybw_name').getEditor().addListener('beforetriggerclick', function () {
        if (Ext.isEmpty(mstform.getItem('PhidTrProj').getValue())) {
            Ext.Msg.alert('提示', "请先选择工程项目");
            return false;
        }
        var pc = mstform.getItem('PhidTrProj').getValue();
        dgrid.getColumn('user_sybw_name').getEditor().setOutFilter({
	...
	dgrid.getColumn('user_cllb_name').getEditor().addListener('beforetriggerclick', function() {
			...
			dgrid.getColumn('user_cllb_name').getEditor().setClientSqlFilter(" u_nm  in (  select YWLBCLLBSET_CLID from cllbdzbfld where  BZZXYWLB_NM in (select gs_nm from p_form0000000257_d where phid= ' " + user_ywlx + " ' )  )");
		});
