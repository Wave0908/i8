    	...
    	Toolbar.insert(19, {
    		...
    		handler: function() {
    			...
    			if (data[0].data.Wrioffflg == '3') {
    				...
    				Ext.Ajax.request({
    					url: "http://192.168.3.72:19082/esey/gyl/pushData/cg_dbthd/"+data[0].data.PhId,
    					method: 'GET',
    					async: false, //同步请求
    				...
    				Ext.Ajax.request({
    					url: "http://192.168.3.72:19082/esey/gyl/pushData/cg_dbrkd/"+data[0].data.PhId,
    					method: 'GET',
    					async: false, //同步请求
  ...
  var Toolbar = Ext.getCmp('toolbar');
	...
	Toolbar.items.get('copy').on('click', function() {
		...
		for(var i = 1; i < a.length; i++) {
			...
			a[i].set('user_zygc_name', user_zygc_name);
			a[i].set('user_ssbk', user_ssbk);
			a[i].set('user_ssbk_name', user_ssbk_name);
