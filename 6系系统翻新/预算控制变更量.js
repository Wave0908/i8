...
function beforeSaveEdit() {
	...
	execServer('yukszl', {
		...
		if (res.count > 0) {
			...
			sum1 += Ext.Number.from(data1[0].extendObjects.qty, 0);
		}
	...
	execServer('yukszl', {
		...
		if (res.count > 0) {
			...
			sum2 += Ext.Number.from(data2[0].extendObjects.qty, 0);
		}
	...
	execServer('yukszl', {
		...
		if (res.count > 0) {
			...
			sum3 += Ext.Number.from(data3[0].extendObjects.qty, 0);
		}
	...
	execServer('yukszl', {
		...
		if (res.count > 0) {
			...
			sum4 += Ext.Number.from(data4[0].extendObjects.qty, 0);
		}
	...
	execServer('yukszl', {
		...
		if (res.count > 0) {
			const data5 = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
			sum5 += Ext.Number.from(data5[0].extendObjects.qty, 0);
		}
		...
		if (res.count > 0) {
			...
			for (var i = 0; i < data.length; i++) {
				if (data[i].extendObjects.u_yskkzlwlkz == '569000000001006') {
					if (flagsum1 == '2') {
						if (sum1 > data[i].extendObjects.qty) {
							Ext.MessageBox.show({
								title: '提示',
