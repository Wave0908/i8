function AllReady() {
	...
	//获取调拨入库的id标识
	var mstform = Ext.getCmp('KC_INKSYSDBRK_Head');
		...
		Toolbar.insert(19, {
			...
			handler: function () {
				...
				if (Wrioffflg == '3') {
					...
					Ext.Ajax.request({
						url: "http://172.20.65.5:30599/new_esey/gyl/pushData/cg_dbthd/" + busid,
						method: 'GET',
						async: false, //同步请求
						disableCaching: false, // 添加这行来阻止_dc参数
						success: function (response) {
					...
					Ext.Ajax.request({
						url: "http://172.20.65.5:30599/new_esey/gyl/pushData/cg_dbrkd/" + busid,
						method: 'GET',
						async: false, //同步请求
						disableCaching: false, // 添加这行来阻止_dc参数
						success: function (response) {
	...
	var dgrid = Ext.getCmp('KC_INKSYSDBRK_Body');
	...
	if (!Ext.isEmpty(Toolbar)) {
		...
		Toolbar.items.get('copy').on('click', function () {
			...
			var user_cllb_name = a[0].get('user_cllb_EXName');
			var user_zygc = a[0].get('user_zygc');
			var user_zygc_name = a[0].get('user_zygc_EXName');
			var user_ssbk = a[0].get('user_ssbk');
			var user_ssbk_name = a[0].get('user_ssbk_EXName');
			var user_dzyhp = a[0].get('user_dzyhp');
			var user_sfzp = a[0].get('user_sfzp');

			for (var i = 1; i < a.length; i++) {
				a[i].set('user_cllb', user_cllb);
				a[i].set('user_cllb_EXName', user_cllb_name);
				a[i].set('user_zygc', user_zygc);
				a[i].set('user_zygc_EXName', user_zygc_name);
				a[i].set('user_ssbk', user_ssbk);
				a[i].set('user_ssbk_EXName', user_ssbk_name);
				a[i].set('user_dzyhp', user_dzyhp);
				a[i].set('user_sfzp', user_sfzp);
			}
		});
	}
	...
	/* 到货验收调拨出库选择前过滤调拨出库以存在的到货验收单start */

	mstform.getItem('user_dhysdbck').on('beforetriggerclick', function (obj) { //帮助窗口打开前事件
		mstform.getItem('user_dhysdbck').setClientSqlFilter(" phid not in (select  user_dhysdbck from kc_billhead where user_dhysdbck  is not null) ");
