$NG.AllReady(function (page,
    { useAction, useImpPullData, useBeforeClick, useBeforeOpen, useValuesChange, useDataIndexChange, useUpdateRows, useClick }
) {
    console.log("========");
    debugger
    ...
    if (proId == '1') {
        ...
        if (gcform.getItem('CatPhId').getValue() != '324191209000058') {
            ...
            gcform.getItem('user_lcywdy').setProps({required: false});
            gcform.getItem('user_mnemcodeinaccdepart').setProps({required: false});
        }
    }
    ...
    if ( $NG.getQueryValue('oType') == 'edit' || $NG.getQueryValue('oType') == 'add') {
        ...
        if (proId == '4') {
            useImpPullData(function ({ args }) {
                // 如果要修改返回的数据，务必保证导入数据的引用地址不变
                console.log("args[0].defaultData:", args[0].defaultData);
                var PhIdSgOrg = args[0].defaultData.PhIdSgOrg
                console.log("PhIdSgOrg:", PhIdSgOrg);
                $NG.execServer('pdsfzhbz', {
                    'phid': PhIdSgOrg
                }, function (res) {
                    if (res.count <= 0) {
                        gcform.getItem('PhIdSgOrg').setValue(null);
                    }
                });
            }, 'formProjectTableMst');
    ...
    if ($NG.getQueryValue('oType') == 'add' || $NG.getQueryValue('oType') == 'edit' ) {
        ...
        if (proId == '4') {
            ...
            //         updater.fieldSetForm.formProjectTableMst.user_gclx1.setProps({
            //             clientSqlFilter: ('grade = 1 and enable_status=1 '),
            //             placeholder: ``
            //         });
            //     });
            // }, "crm3.wbs_type");
            useBeforeOpen(async function ({ args }) {
                ...
                console.log("过滤user_gclx2的user_gclx1:", user_gclx1);
                if (user_gclx1) {
                    $NG.updateUI(function (updater, state) {
                        updater.fieldSetForm.formProjectTableMst.user_gclx2.setProps({
                            clientSqlFilter: ('grade = 2 and pphid =' + user_gclx1 + ' and enable_status=1'),
                            placeholder: ``
                        });
                    });
                } else {
    ...
    //                 Ext.Msg.alert('提示服务端取数失败');
    //                 return;
    //             } else if (res.count == 0) { //判断数组行数
    //                 Ext.Msg.alert('提示项目立项状态编码失败');
    //                 return;
    //             }
    //             if (res.count > 0) {
    ...
    useBeforeOpen(async function ({ args }) {
        if (gcform) {
            ...
            console.log("工程 管理组织:", zz);
            var ocode = gcform.getItem('PhIdFiOcode').getValue();
            console.log("工程默认财务组织:", ocode);
            if (zz) {
                $NG.updateUI(function (updater, state) {
                    console.log("user_pc_dept updater:", updater);
            ...
            if (ocode) {
                $NG.updateUI(function (updater, state) {
                    updater.fieldSetForm.formProjectTableMst.user_cwglbm.setProps({
                        clientSqlFilter: ('parent_orgid = ' + ocode),
                        placeholder: ``
                    });
                });
            }
        } else if (xnform) {
            var xnzz = xnform.getItem('CatPhId').getValue();
            console.log("虚拟 管理组织：", xnzz);
            $NG.updateUI(function (updater, state) {
                updater.fieldSetForm.formProjectTableVirtProj.user_pc_dept.setProps({
                    clientSqlFilter: ('parent_orgid = ' + xnzz),
                    placeholder: ``
                });
            });
        }
        ...
        if ($NG.getQueryValue('oType') == 'add') {
            try {
                ...
                if (resFjm.status != 'success') {
                    $NG.alert('提示服务端取数失败(user_gs_fjm)');
                    return false;
                }
                if (resFjm.count > 0) {
                    ...
                    $NG.alert('提示项目立项状态编码失败(user_gs_fjm)');
                    return false;
                }
