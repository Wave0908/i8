$NG.AllReady(function (page,
	...
	var dToolbar = $NG.getCmpApi('CntProjBalDToolbar');
	console.log("dToolbar==========", dToolbar)
	console.log("dToolbar.getButtons==========", dToolbar.getButtons())
	dToolbar.getItem('addrow').setReadOnly(true);
	...
	useValuesChange(({
		...
		var Paytype = mstform.getItem('Paytype').getValue();
		if (!Paytype) {
			return false;
		}
		if (Paytype == '0') {
			...
			dToolbar.getItem('addrow').setReadOnly(false);
		}
	}, 'ProjBalM6.Paytype');
	/*价税合同改变触发start*/
	useValuesChange(({
