	...
	if (otype == $Otype.ADD) {
		Toolbar.items.get('save').on('click', function() { //点击按钮检测事件
			...
			if (mstform) {
				...
				for (var i = 0; i < mstform.fields.length; i++) {
					if (mstform.fields[i].mustInput) { //获取必输项字段
						if (Ext.isEmpty(mstform.getItem(mstform.fields[i].name).getValue())) {
							temp = 0;
							return;
						}
					};
			...
			execServer('hsxmnm', {}, function(res) {
				if (res.status != 'success') {
					...
					return;
				} else if (res.data.length == 0) { //判断数组行数
					Ext.Msg.alert('提示', '项目立项状态编码失败');
					return;
				}
				var num = parseInt(res.data[0].hsxmnm) + 1
			...
			execServer('user_gs_fjm', {}, function(res) {
				...
				var num = parseInt(res.data[0].user_gs_fjm) + 1
	...
	if (mstform1) {
		...
		mstform1.getItem('user_pc_dept').addListener('helpselected', function() {
			...
			execServer('ssxmb_bmywdy', {
				...
				if (res.data[0]) {
					mstform1.getItem('user_lcywdy').setValue(res.data[0].user_lcywdy);
				} else {
			...
			execServer('ssxmb_zjm', {
				...
				if (res.data[0]) {
					mstform1.getItem('user_mnemCodeInAccDepart').setValue(res.data[0]
						.user_mnemCodeInAccDepart);
				} else {
					Ext.Msg.alert('提示', '浪潮核算部门的助记码没有和新中大部门编码保持一致');
					return false;
		...
		execServer('pt.yfxm', {}, function(res) {
			var num = parseInt(res.data[0].num) + 1;
			var date = new Date();
