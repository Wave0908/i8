function AllReady() {
    var mstform = Ext.getCmp('KC_OUTCGTH_Head');
                ...
                Ext.Ajax.request({
                    ...
                    success: function (response) {
                        ...
                        if (message == "调用成功") {
                            ...
                            Ext.MessageBox.alert(Lang.Notes || '提示', "推送失败", function () { });
                        }
    ...
    var dgrid = Ext.getCmp('KC_OUTCGTH_Body');
    ...
    mstform.getItem('user_ywlx').on('beforetriggerclick', function (eOp, ignoreBeforeEvent) { //帮助窗口打开前事件
        mstform.getItem('user_ywlx').setClientSqlFilter(" zd='业务类型' and djmc='发票报账单' ");
    ...
    mstform.getItem('user_kclx').on('change', function (eOp, ignoreBeforeEvent) { //帮助窗口打开前事件
        ...
        if (user_kclx == '1') {
            mstform.getItem('user_kkxz').userSetMustInput(true);
        } else {
            mstform.getItem('user_kkxz').userSetMustInput(false);
        }
    ...
    dgrid.getColumn('user_fpfyxm_EXName').getEditor().addListener('beforetriggerclick', function () {
        var user_ywlx = mstform.getItem('user_ywlx').getValue();
        if (Ext.isEmpty(user_ywlx)) {
        ...
        dgrid.getColumn('user_fpfyxm_EXName').getEditor().setClientSqlFilter(" u_gsywlxnm in (select gs_nm from  p_form0000000257_d  where  phid =" + user_ywlx + ")");
    });
    ...
    Toolbar.items.get('copy').on('click', function () {
        ...
        var user_zygc_name = a[0].get('user_zygc_name');
        var user_ssbk = a[0].get('user_ssbk');
        var user_ssbk_name = a[0].get('user_ssbk_name');
        var user_fpfyxm = a[0].get('user_fpfyxm');
        var user_fpfyxm_name = a[0].get('user_fpfyxm_EXName');
        ...
        for (var i = 1; i < a.length; i++) {
            ...
            a[i].set('user_fpfyxm', user_fpfyxm);
            a[i].set('user_fpfyxm_EXName', user_fpfyxm_name);
