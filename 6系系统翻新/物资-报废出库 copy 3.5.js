function AllReady() {
	...
	// 开发提议 所有的出入库表单加上这个二开
	if (jsPropMap && jsPropMap['kc_billhead'] && jsPropMap['kc_billhead'].PhidTrProj) {
		delete jsPropMap['kc_billhead'].PhidTrProj;
	}
	// 开发提议 所有的出入库表单加上这个二开

    var mstform = Ext.getCmp('KC_OUTBFTH_Head');
    ...
    if (otype == $Otype.VIEW) {
        Toolbar.insert(18, {
            ...
            handler: function () {
                ...
                // if (!Ext.isEmpty(user_istbinspur) || user_istbinspur == '1') {
        ...
        if (Toolbar) {
            Toolbar.insert(19, {
                ...
                handler: function () {
                    var ChkFlg = mstform.queryById('ChkFlg').getValue();
                    var user_cklx = mstform.queryById('user_cklx').getValue();
                    if (Ext.isEmpty(ChkFlg) || ChkFlg == '0') {
                        Ext.Msg.alert('提示', '当前单据未审核，审核后重新推送');
                        return false;
                    }
                    if (user_cklx == '1') {
                        ...
                        Ext.Ajax.request({
                            url: "http://172.20.65.5:30599/new_esey/gyl/pushData/cg_bsd/" + busid,
                            method: 'GET',
                            async: false, //同步请求
                            disableCaching: false, // 添加这行来阻止_dc参数
                            success: function (response) {
                        ...
                        Ext.Ajax.request({
                            url: "http://172.20.65.5:30599/new_esey/gyl/pushData/cg_bfd/" + busid,
                            method: 'GET',
                            async: false, //同步请求
                            disableCaching: false, // 添加这行来阻止_dc参数
                            success: function (response) {
    ...
    var dgrid = Ext.getCmp('KC_OUTBFTH_Body');
