$NG.AllReady(function (page,
    ...
    useValuesChange(({
        ...
        if (user_lcssxmb == null || user_lcssxmb == '') {
            return;
        }
        $NG.execServer('ssxmb_bmywdy', {
            'dept': user_lcssxmb
    ...
    $NG.updateUI(function (updater, state) {
        updater.fieldSetForm.PcmM1.PhidPc.setProps({
            clientSqlFilter: (" phid  not in ( select phid_pc from pcm3_cnt_m where cnt_type in ('5','224200107000001','224200107000002','224200107000003') group by phid_pc) "),
            placeholder: ``
        });
    });
