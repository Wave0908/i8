function AllReady() {
    ...
    if (otype == $Otype.ADD) {
        ...
        mstform.getItem('user_jtnbry').addListener('helpselected', function (obj) {
            console.log('obj=============>', obj)
            var h6w = obj.data.id_card;
            var name = obj.data.lastname;
            const compname = name + String(h6w).slice(-6);
            mstform.getItem('CompName').setValue(compname);
...
function beforeSaveEdit() {
    ...
    var CompName = mstform.getItem('CompName').getValue() ? "'" + mstform.getItem('CompName').getValue() + "'" : mstform.getItem('CompName').getValue();
    //var dstore = dgrid.store;
    var flag = 1;
