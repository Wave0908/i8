var tempFields = individualInfo.form[gc_gobal.bustype + '_Head'].fields.filter(f => f.itemId == 'WgTaxmony');
if (Array.isArray(tempFields) && tempFields.length > 0) {
    delete tempFields[0].allowDecimals;
}
function AllReady() {

    //获取调拨出库出库的id标识
    var mstform = Ext.getCmp('KC_OUTDBCK_Head');
        ...
        if (!Ext.isEmpty(Toolbar)) {
            Toolbar.insert(19, {
                ...
                handler: function () {
                    ...
                    if (Wrioffflg == '3') {
                        ...
                        Ext.Ajax.request({
                            url: "http://172.20.65.5:30599/new_esey/gyl/pushData/cg_dbtkd/" + busid,
                            method: 'GET',
                            async: false, //同步请求
                            disableCaching: false, // 添加这行来阻止_dc参数
                            success: function (response) {
                        ...
                        Ext.Ajax.request({
                            url: "http://172.20.65.5:30599/new_esey/gyl/pushData/cg_dbckd/" + busid,
                            method: 'GET',
                            async: false, //同步请求
                            disableCaching: false, // 添加这行来阻止_dc参数
                            success: function (response) {
            ...
            Toolbar.insert(20, {
                ...
                handler: function () {
                    ...
                    Ext.Ajax.request({
                        url: "http://172.20.65.5:30599/new_esey/d6c/D6cpushData/d6c_dbdxxzb/d6c_dbdxxzb/" + busid,
                        method: 'GET',
                        async: false, //同步请求
                        disableCaching: false, // 添加这行来阻止_dc参数
                        success: function (response) {
    ...
    var dgrid = Ext.getCmp('KC_OUTDBCK_Body');
    ...
    dgrid.setReadOnlyCol('DbTaxamt', false)
    //单据编号设置为必输
    ...
    if (otype == $Otype.ADD || otype == $Otype.EDIT || otype == $Otype.VIEW) {
        var autoCalcDbse = function () {
            Ext.Array.each(dstore.data.items, function (record) {
                var rest = record.data
                //调拨价税合计-调拨金额=调拨税额
                var dbjshj = rest.DbTaxprc * rest.OrQty;
                var bhsje = dbjshj / (1 + rest.Taxrate)
                var dbse = dbjshj - bhsje

                if (rest.user_dbse - dbse < 0.06 && rest.user_dbse != 0 || dbse - rest.user_dbse < 0.06 && rest.user_dbse != 0) {
                    record.set('user_dbse', Ext.Number.from(rest.user_dbse, 0).toFixed(2));
                    record.set('RkSum', Ext.Number.from(dbjshj - rest.user_dbse, 0).toFixed(2));
                    record.set('DbTaxamt', Ext.Number.from(dbjshj, 0).toFixed(2));
                } else {
                    record.set('user_dbse', Ext.Number.from(dbse, 0).toFixed(2));
                }
            });
        };

        // 监听 update: 任何字段被修改（无论是用户手动编辑，还是代码 record.set()）都会触发
        dstore.on('update', autoCalcDbse);
        // 监听 datachanged: 任何行的增加、删除、重新加载都会触发
        dstore.on('datachanged', autoCalcDbse);
    }
