$NG.AllReady(function (
    ...
    if (isChgCnt == '1') {
        ...
        mstform.getItem('user_rate').setProps({ disabled: true, required: false });
    ...
    useValuesChange(({
        ...
        console.log('PhidSenComp---------->', PhidSenComp)
    ...
    //         var row = rows[i];
    //         total_sum += row.AmtVatFc;
    //     }
    //     console.log('计算后======》', total_sum);
    // }, "PcmD5");
    ...
    useBeforeClick(({
        ...
        if (isChgCnt == '1') {
            const CntOrgSumVatFc = mstform.getItem('CntOrgSumVatFc').getValue();
            const CntChgSumVatFc = mstform.getItem('CntSumVatFc').getValue();
            console.log('原含税金额=====', CntOrgSumVatFc);
            console.log('变更后含税金额=====', CntChgSumVatFc);
            // 获取结算额
            if (CntChgSumVatFc > CntOrgSumVatFc * 1.1) {
                $NG.alert('变更后合同金额不能大于原合同金额的110%');
                flag = 1;
                return false;
            } else {
                flag = 0;
