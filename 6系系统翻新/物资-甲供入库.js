function AllReady() {
	...
	mstform.getItem('PhidTrProj').addListener('helpselected', function (obj) {
		...
		execServer('gjxmxx_dcssxmb', {
			...
			if (data.length != 0 && data[0].extendObjects.user_pc_dept) {
		...
		if (pc) {
			/**通过项目带出合同信息 */
			execServer('get_ifdswzkpt', { 'phid_pc': pc }, function (res) {
				if (res.count > 0) {
					var data = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
					console.log('data====================>', data)
					console.log('user_sfsydswzkpt====================>', data[0].extendObjects.user_sfsydswzkpt)
					if (data[0].extendObjects.user_sfsydswzkpt == '01') {
						mstform.getItem('user_sfsydswzkpt').setValue('01');
					} else {
						mstform.getItem('user_sfsydswzkpt').setValue('02');
					}
				}
			});
		}
	...
	console.log('DefaultPc', DefaultPc)
	if (DefaultPc) {
