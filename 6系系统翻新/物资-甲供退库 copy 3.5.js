function AllReady() {
    ...
    var mstform = Ext.getCmp('KC_INKSYSJGRK_Head');
    ...
    var dgrid = Ext.getCmp('KC_INKSYSJGRK_Body');
    ...
    if (otype == $Otype.ADD) {
        mstform.on('dataready', function () {
            //默认价格类型为实价    
            mstform.getItem('Tempflg').setValue(i.toString());
        });
    }
    ...
    mstform.getItem('PhidTrProj').addListener('helpselected', function (obj) {
        ...
        execServer('gjxmxx_dcssxmb', {
            ...
            if (res.count > 0) {
                var resource = JSON.parse(res.data);
                if (!Ext.isEmpty(resource[0].extendObjects.user_pc_dept)) {
                    execServer('ssxmb_bmywdy', {
                        'dept': resource[0].extendObjects.user_pc_dept
                    }, function (res) {
                        if (res.count > 0) {
                            ...
                            if (data.length == 1) {
                                ...
                                stform.getItem('user_lcywdy').setValue(null);
                                return false;
                            }
                        } else {
                            Ext.Msg.alert('提示', '请联系管理员处理，部门对照未做');
    ...
    if (!Ext.isEmpty(pc)) {
        ...
        execServer('gjxmxx_dcssxmb', {
            ...
            if (res.count > 0) {
                var recourse = JSON.parse(res.data);
                if (!Ext.isEmpty(recourse[0].extendObjects.user_pc_dept)) {
                    execServer('ssxmb_bmywdy', {
                        'dept': recourse[0].extendObjects.user_pc_dept
                    }, function (res) {
                        if (res.count > 0) {
                            const data = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
                            if (data.length == 1) {
