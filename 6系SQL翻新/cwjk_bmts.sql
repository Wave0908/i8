...
FROM
  ...
  LEFT JOIN (
    SELECT DISTINCT
      pm_zzbm,
      ztbm = STUFF ((SELECT ',' + ztbm FROM p_form0000000215_d WHERE pm_zzbm = d.pm_zzbm FOR XML PATH('')), 1, 1, '')
        FROM
          p_form0000000215_d d
      ) ztzh ON d.ocode = ztzh.pm_zzbm /*账套转换*/
    ...
    WHERE
      ...
      AND d.phid NOT IN (SELECT xzd_bmmc FROM p_form0000000217_d WHERE xzd_bmmc IS NOT NULL) UNION
      /*创建项目部推送生成业务单元下的核算部门*/
    SELECT
      d.ocode 所属组织,
      d.phid 部门主键,
        ...
        FROM
          ...
          LEFT JOIN (
            SELECT DISTINCT
              ...
              LEFT JOIN p_form0000000223_d p223d ON ztzh.ztbm = p223d.dwbh
              LEFT JOIN (SELECT phid, dwbh, dwmc, sjjg FROM p_form0000000223_d WHERE dwmc LIKE'%事业部机关') sybjg ON sybjg.sjjg = p223d.dwbh /*取事业部机关*/
              LEFT JOIN (SELECT ocode, parentorg FROM fg_orgrelatitem WHERE relatid = '18') sjbm ON d.deptno = sjbm.ocode
              LEFT JOIN fg_orglist fg ON d.deptno = fg.ocode
            WHERE
              fg.ng_insert_dt > '2021-08-06 00:30:37.000'
                ...
                FROM
                  ...
                  LEFT JOIN (
                    ...
                    WHERE
                      ...
                      AND (sjbm.parentorg LIKE'%.1004')
                      AND d.phid NOT IN (SELECT xzd_bmmc FROM p_form0000000217_d WHERE xzd_bmmc IS NOT NULL) UNION
                      /*创建分公司厂处推送上级部门对应事业部机关业务单元下增加核算部门*/
                    SELECT
                      d.ocode 所属组织,
                      d.phid 部门主键,
                      d.deptno 部门编码,
