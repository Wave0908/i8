...
$NG.AllReady(function (
            ...
            if (u_gcxm) {
                ...
                $NG.updateUI(function (updater, state) {
                    ...
                    updater.fieldSetForm.p_form_liangjintianbao_m.u_tongjizhouqi.setProps({
                        clientSqlFilter: (`ctype = 'GCMONTH'  `),	//根据
                        placeholder: ``
                    });
                });
                /**监听项目选择后过滤统计周期 */
                ...
                $NG.execServer("ljht", { pc: phid_pc }, async function (res) {
                    ...
                    if (res.count > 0) {
                        ...
                        for (let i = 0; i < data.length; i++) {
                            ...
                            const user_zbjqx = 12 * Number(data[i].extendObjects.user_zbjqx)
    ...
    useBeforeClick(async function ({ args }) {
        ...
        for (let k = 0; k < rows.length; k++) {
            ...
            const zlhj = (Number(row.u_month2 || 0) + Number(row.u_month3_6 || 0) + Number(row.u_month7_12 || 0) + Number(row.u_year1_2 || 0) + Number(row.u_year2_3 || 0) + Number(row.u_year3up || 0)).toFixed(2);
            const sjyszk = Number(row.u_sjyszk || 0).toFixed(2);

            console.log("第" + (k + 1) + "行，保留两位小数，累计开单➖累计回款 = " + diff);
            console.log("第" + (k + 1) + "行，应收账款小计 = " + sjyszk);
            console.log("第" + (k + 1) + "行，账龄合计 = " + zlhj);
            if (sjyszk < 0) {
                return true;
            } else if (zlhj != sjyszk) {
